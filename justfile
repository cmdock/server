...
check: fmt-check lint machete codec-gate test

# MergedSyncGateway codec corpus + TaskChampion-version bump guard.
codec-gate:
    scripts/check-merged-sync-codec-gate.sh

# ADR-0002 §HC-2 import-boundary enforcement.
# Catches both `use crate::X::` and fully-qualified `crate::X::` paths
# (the 2026-05-04 review found two fully-qualified-path violations the
# original use-only sweep missed; see server#128). Currently NOT in
# `just check` because two pre-existing violations are tracked under
# server#123 (devices → tc_sync) and the tasks → views::defaults fix.
# Add to `just check` once those land.
hc2:
    scripts/check-hc2-boundaries.sh
...
# Run the Tier 1 examples smoke gate against the candidate server image.
# Requires a local cmdock/examples checkout (defaults to ../cmdock-examples)
# and a server image tag (defaults to ghcr.io/cmdock/server:latest).
qualify-examples examples_path='../cmdock-examples' server_image='ghcr.io/cmdock/server:latest':
    python3 ./scripts/release_qualification.py \
        --examples-repo-path {{ examples_path }} \
...
# Run full E2E staging tests with public webhook receiver and strict summary parsing.
staging-full:
    #!/usr/bin/env bash
    set -uo pipefail
    tmp="$(mktemp)"
    trap 'rm -f "$tmp"' EXIT
    receiver="${WEBHOOK_RECEIVER_EXTERNAL_URL:-${CMDOCK_WEBHOOK_RECEIVER_URL:-${STAGING_WEBHOOK_RECEIVER_URL:-https://webhook-staging.cmdock.dev}}}"
    WEBHOOK_RECEIVER_EXTERNAL_URL="$receiver" ./scripts/staging-test.sh --full --url "{{ staging_url }}" --ssh "{{ staging_ssh }}" 2>&1 | tee "$tmp"
    test_status=${PIPESTATUS[0]}
    scripts/staging_summarize.sh --require-no-skips --require-webhook-pass "$tmp"
    summary_status=$?
...
# Admin runs over SSH; the local `task` CLI needs to reach the server's TLS
# cert — run on the staging host, or use real-sync-test-tunneled from a
# workstation (the internal CA is not in a typical workstation trust store).
real-sync-test users="3" tasks="5":
    ./scripts/real-sync-load.sh {{ users }} {{ tasks }}

# Workstation variant: SSH-tunnel the server's plain-HTTP port and run the real
# sync load test over it (no internal-CA trust needed locally). (#145)
real-sync-test-tunneled users="3" tasks="5":
    #!/usr/bin/env bash
    set -euo pipefail
    ssh -o ControlMaster=no -o ExitOnForwardFailure=yes -N -L "{{ staging_tunnel_port }}:127.0.0.1:8080" "{{ staging_ssh }}" &
    TUNNEL_PID=$!
    trap 'kill "$TUNNEL_PID" >/dev/null 2>&1 || true; wait "$TUNNEL_PID" >/dev/null 2>&1 || true' EXIT
    # Wait for the forward to actually carry traffic instead of a fixed sleep.
    for _ in $(seq 1 20); do
        curl -fsS -m 2 "{{ staging_tunnel_url }}/healthz" >/dev/null 2>&1 && break
        sleep 0.5
    done
    curl -fsS -m 2 "{{ staging_tunnel_url }}/healthz" >/dev/null \
        || { echo "tunnel not ready ({{ staging_tunnel_url }}/healthz unreachable)"; exit 1; }
    STAGING_SSH="{{ staging_ssh }}" ./scripts/real-sync-load.sh {{ users }} {{ tasks }} "{{ staging_tunnel_url }}"

# Run real drift recovery + orphan reconciliation E2E test on staging
# (#130 Phase 5b/5e). Runs on the staging host by default (local
# `sudo docker compose exec`); from a workstation use
# real-drift-orphan-test-tunneled. See script header for prerequisites.
real-drift-orphan-test url=staging_url:
    ./scripts/real-drift-orphan-test.sh "{{ url }}"

# Workstation variant: SSH-tunnel the server's plain-HTTP port (for the local
# `task` CLI) and route admin + log reads over SSH via STAGING_SSH. (#145)
real-drift-orphan-test-tunneled:
    #!/usr/bin/env bash
    set -euo pipefail
    ssh -o ControlMaster=no -o ExitOnForwardFailure=yes -N -L "{{ staging_tunnel_port }}:127.0.0.1:8080" "{{ staging_ssh }}" &
    TUNNEL_PID=$!
    ...
    curl -fsS -m 2 "{{ staging_tunnel_url }}/healthz" >/dev/null \
        || { echo "tunnel not ready ({{ staging_tunnel_url }}/healthz unreachable)"; exit 1; }
    STAGING_SSH="{{ staging_ssh }}" ./scripts/real-drift-orphan-test.sh "{{ staging_tunnel_url }}"

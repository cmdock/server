...
/// context_id is serialised on GET /api/views for project-scoped named views.
#[tokio::test]
async fn test_views_response_includes_context_id() {
    let env = setup().await;
    let (h, v) = auth_header(&env.user_a.token);
    ...
    // Project-scoped named views should have contextId set
    let work = views.iter().find(|v| v["id"] == "work").unwrap();
    assert_eq!(
        work["contextId"], "work",
        "work view should have contextId=work"
    );
    assert_eq!(work["contextFiltered"], true);
    ...
    let health = views.iter().find(|v| v["id"] == "health").unwrap();
    assert_eq!(health["contextId"], "health");
    assert_eq!(health["contextFiltered"], true);

    // Cross-cutting views should NOT have contextId
    let duesoon = views.iter().find(|v| v["id"] == "duesoon").unwrap();
    assert!(
        duesoon.get("contextId").is_none() || duesoon["contextId"].is_null(),
        "cross-cutting duesoon should not have contextId"
    );
    assert_eq!(duesoon["contextFiltered"], true);

    // Tag-scoped views should NOT have contextId
    let shopping = views.iter().find(|v| v["id"] == "shopping").unwrap();
    assert!(shopping.get("contextId").is_none() || shopping["contextId"].is_null());
    assert_eq!(shopping["contextFiltered"], false);
}

/// context_id is serialised on GET /api/app-config for project-scoped named views.
#[tokio::test]
async fn test_app_config_views_include_context_id() {
    let env = setup().await;
    let (h, v) = auth_header(&env.user_a.token);

    // Trigger reconcile via views endpoint first
    let _ = env
        .server
        .get("/api/views")
        .add_header(h.clone(), v.clone())
    ...
    let views = config["views"].as_array().unwrap();

    let work = views.iter().find(|v| v["id"] == "work").unwrap();
    assert_eq!(work["contextId"], "work");
    assert_eq!(work["contextFiltered"], true);
...
/// VIEWSET 4→5 reconciliation updates unmodified named views to context_filtered + context_id.
#[tokio::test]
async fn test_reconciliation_updates_stale_named_views() {
    let env = setup().await;

    // Seed a v4-style work view directly (context_filtered=false, no context_id)
    let stale_work = cmdock_server::store::models::ViewRecord {
        id: "work".to_string(),
        label: "Work".to_string(),
        icon: "briefcase".to_string(),
        filter: "status:pending project:WORK".to_string(),
        ...
        user_modified: false,
        hidden: false,
        template_version: cmdock_server::views::defaults::VIEWSET_VERSION - 1,
        context_id: None,
    };
    env.store
        .upsert_view(&env.user_a.user_id, &stale_work)
        .await
        .unwrap();

    // GET /api/views triggers reconcile
    let (h, v) = auth_header(&env.user_a.token);
    let resp = env.server.get("/api/views").add_header(h, v).await;
    resp.assert_status_ok();
    let views: Vec<Value> = resp.json();
    ...
    let work = views.iter().find(|v| v["id"] == "work").unwrap();
    assert_eq!(
        work["contextFiltered"], true,
        "reconciled work should be context_filtered"
    );
    assert_eq!(
        work["contextId"], "work",
        "reconciled work should have context_id"
    );
    assert_eq!(
        work["filter"], "status:pending",
        "reconciled work filter should be broadened"
    );
}

/// User-modified views are NOT updated by reconciliation.
#[tokio::test]
async fn test_reconciliation_preserves_user_modified_views() {
    let env = setup().await;

    // Seed a user-modified work view with old template version
    let modified_work = cmdock_server::store::models::ViewRecord {
        id: "work".to_string(),
        label: "My Work".to_string(),
        icon: "briefcase".to_string(),
        filter: "status:pending project:MYWORK".to_string(),
        group_by: Some("project".to_string()),
        context_filtered: false,
        display_mode: "grouped".to_string(),
        sort_order: 40,
    ...
    env.store
        .upsert_view(&env.user_a.user_id, &modified_work)
        .await
        .unwrap();
    ...
    let work = views.iter().find(|v| v["id"] == "work").unwrap();
    assert_eq!(work["label"], "My Work", "user label should be preserved");
    assert_eq!(
        work["filter"], "status:pending project:MYWORK",
        "user filter should be preserved"
    );
    // context_filtered and context_id are server-owned — merged even on user-modified views
    assert_eq!(
        work["contextFiltered"], true,
        "context_filtered is server-owned, should be updated"
    );
    assert_eq!(
        work["contextId"], "work",
        "context_id is server-owned, should be set"
    );
}

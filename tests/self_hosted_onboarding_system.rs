...
async fn start_sync_server(data_dir: std::path::PathBuf, db_path: &std::path::Path) -> TestServer {
    ...
    let config = common::test_server_config_with_master_key(data_dir, [0x2a; 32]);
...
async fn test_self_hosted_offline_cli_device_onboarding_flow() {
    ...
    let parent = Uuid::nil();
    let first_history = valid_history_segment();
    let encrypted = cryptor.seal(parent, &first_history).unwrap();
    ...
    assert_eq!(decrypted, first_history);
    ...
    let second_history = valid_history_segment();
    let encrypted = cryptor.seal(first_version, &second_history).unwrap();
    let (ch, cv) = client_id_header(&client_id);
    let resp = server
        .post(&format!("/v1/client/add-version/{first_version}"))
        .add_header(ch, cv)
    ...
    let decrypted = cryptor
        ...
        .unwrap();
    assert_eq!(decrypted, second_history);
...
async fn test_device_registration_and_revocation_survive_restart() {
    ...
    let parent = Uuid::nil();
    let history = valid_history_segment();
    let encrypted = active_cryptor.seal(parent, &history).unwrap();

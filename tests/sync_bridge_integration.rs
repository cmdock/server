...
/// Test that creating a task via REST API produces a version in the served merged sync chain.
///
/// Flow: POST /api/tasks → GET /v1/client/get-child-version/{nil} triggers gateway projection.
#[tokio::test]
async fn test_rest_create_appears_in_sync_chain() {
    ...
    // Should return 200 with a version (gateway projection appended the REST change).
    resp.assert_status_ok();
    ...
    assert!(
        !env._tmp
            ...
            .join("sync.sqlite")
            .exists(),
        "REST writes must not warm the legacy non-serving sync.sqlite chain after gateway cutover"
    );
}
...
async fn test_no_master_key_skips_sync() {
    ...
    // Phase 6 cutover: the gateway is the runtime sync path. With no master
    // key, the protocol is plaintext but REST writes still project to the
    // merged chain; the old "bridge skips when unencrypted" behaviour no
    // longer means an empty `/v1/client/*` chain.
    let nil = Uuid::nil();
    let (ch, cv) = client_id_header(&client_id);
    let resp = server
        .get(&format!("/v1/client/get-child-version/{nil}"))

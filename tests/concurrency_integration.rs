...
async fn test_sync_storage_corruption_server_stays_responsive() {
    ...
    // Ensure the merged sync DB exists by letting the gateway project the REST write.
    let nil = Uuid::nil();
    let (ch, cv) = client_id_header(&env.user_a.client_id);
    let push_resp = env
        .server
    ...
    // Corrupt the gateway merged sync storage file. The legacy
    // users/{id}/sync.sqlite path is no longer served after Phase 6 cutover.
    let sync_db = env
        ._tmp
        .path()
        .join("users")
    ...
    assert!(
        sync_db.exists(),
        "merged sync DB should exist after pushing a version"
    );

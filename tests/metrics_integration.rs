...
async fn setup_test_server_with_webhook_metrics() -> (TestServer, String, AppState) {
    setup_test_server_with_webhook_metrics_capacity(None).await
}

/// As [`setup_test_server_with_webhook_metrics`], but pins the async webhook
/// dispatch capacity (#156) so a test can saturate it and observe the shed
/// counter. The override is applied before the router captures its `AppState`
/// clone (the handler reads the tracker through that shared `Arc`).
async fn setup_test_server_with_webhook_metrics_capacity(
    dispatch_capacity: Option<usize>,
) -> (TestServer, String, AppState) {
    let tmp = tempfile::tempdir().unwrap();
    let data_dir = tmp.path().to_path_buf();
    std::fs::create_dir_all(data_dir.join("users")).unwrap();
...
async fn test_metrics_expose_webhook_delivery_metrics() {
    ...
    // Webhook delivery is dispatched asynchronously (#149); wait for it to
    // finish so the delivery counters are recorded before scraping /metrics.
    assert!(
        state
            .webhook_dispatch
...
#[tokio::test]
async fn test_metrics_record_webhook_dispatch_shed_at_capacity() {
    // #156: when async webhook dispatch is at capacity, finalize_success sheds
    // the event and increments `webhook_dispatch_shed_total` — the main operator
    // signal that delivery is being dropped. Capacity 1 + a held permit forces
    // the next mutation to shed.
    let _guard = metrics_test_guard();
    let (server, token, state) = setup_test_server_with_webhook_metrics_capacity(Some(1)).await;
    let (hdr_name, hdr_val) = auth_header(&token);
    ...
    server
        ...
        .add_header(hdr_name.clone(), hdr_val.clone())
        .json(&serde_json::json!({
            "url": "https://hooks.example.invalid/shed",
            "secret": "abcdefghijklmnopqrstuvwxyz0123456789",
            "events": ["task.created"]
        }))
        .await
    ...
    // Occupy the single dispatch slot — the next mutation's event must shed.
    let hold = state
        .webhook_dispatch
        .try_enter()
        .expect("capacity-1 tracker admits the first entry");

    // The global recorder accumulates across the (serialized) metrics tests, so
    // assert on the delta rather than an absolute value.
    let shed_value = |body: &str| -> f64 {
        body.lines()
            .find_map(|l| l.strip_prefix("webhook_dispatch_shed_total "))
            .and_then(|v| v.trim().parse::<f64>().ok())
            .unwrap_or(0.0)
    };
    let before = shed_value(&server.get("/metrics").await.text());

    server
        .post("/api/tasks")
        .add_header(hdr_name, hdr_val)
        .json(&serde_json::json!({"raw": "+shed metrics shed me"}))
        .await
        .assert_status_ok();

    // Shedding is synchronous within finalize_success — the counter is already
    // incremented by the time the 2xx returns (no quiescence wait needed).
    let after_body = server.get("/metrics").await.text();
    assert!(
        after_body.contains("webhook_dispatch_shed_total"),
        "shed counter must be exposed once a shed has occurred"
    );
    assert_eq!(
        shed_value(&after_body),
        before + 1.0,
        "exactly one shed must be recorded for the at-capacity mutation"
    );

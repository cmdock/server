...
async fn test_register_and_list_device() {
    ...
    assert!(
        merged_path.exists(),
        "device registration should initialise the merged gateway sync DB"
    );
...
#[tokio::test]
async fn test_legacy_canonical_sync_identity_client_id_forbidden() {
    let (server, store, user_id, _token, _tmp) = setup().await;
    let canonical = store
        .get_replica_by_user(&user_id)
        .await
        .unwrap()
        .expect("setup creates a canonical sync identity");

    let (ch, cv) = client_id_header(&canonical.id);
    let resp = server
        .post(&format!("/v1/client/add-version/{}", Uuid::nil()))
        .add_header(ch, cv)
        .add_header(header::CONTENT_TYPE, HeaderValue::from_static(HS_CT))
...
async fn test_registered_device_encrypted_sync_round_trip() {
    let (server, _store, _user_id, token, _tmp) = setup().await;
    let (h, v) = auth_header(&token);
    let (client_id, secret) = register_device(&server, &h, &v, "Encrypted Sync Device").await;
    let client_uuid = Uuid::parse_str(&client_id).unwrap();
    let cryptor = SyncCryptor::new(client_uuid, secret.as_bytes()).unwrap();

    let parent = Uuid::nil();
    let history = valid_history_segment();
    let encrypted = cryptor.seal(parent, &history).unwrap();
...
async fn test_registered_device_encrypted_snapshot_round_trip() {
    let (server, _store, _user_id, token, _tmp) = setup().await;
    let (h, v) = auth_header(&token);
    let (client_id, secret) = register_device(&server, &h, &v, "Encrypted Snapshot Device").await;
    let client_uuid = Uuid::parse_str(&client_id).unwrap();
    let cryptor = SyncCryptor::new(client_uuid, secret.as_bytes()).unwrap();

    let parent = Uuid::nil();
    let encrypted_history = cryptor.seal(parent, &valid_history_segment()).unwrap();

...
async fn test_quarantine_blocks_all_task_endpoints() {
    ...
    // Use a valid ModifyTaskRequest body so strict-recognise (deny_unknown_fields,
    // server#100) doesn't short-circuit with 400 before quarantine fires.
    let (h, v) = auth_header(&env.user_a.token);
    let resp = env
        .server
        .post(&format!("/api/tasks/{dummy_uuid}/modify"))
        .add_header(h, v)
        .json(&serde_json::json!({"priority": "M"}))
        .await;
    resp.assert_status(axum::http::StatusCode::SERVICE_UNAVAILABLE);
...
async fn test_integrity_check_healthy_db() {
    ...
    let sync_result = sync_results
        ...
        .find(|entry| entry.starts_with("merged/sync.sqlite:"))
        .expect("integrity_check.sync should include merged sync.sqlite");
    assert!(
        sync_result == "merged/sync.sqlite: ok",
        "Healthy merged sync DB should report ok, got: {sync_result}"
    );
}
...
async fn test_integrity_check_corrupt_db() {
    ...
    // Also corrupt the merged sync DB and verify integrity_check.sync reports an error.
    let sync_db = replica_dir.join("merged").join("sync.sqlite");
    std::fs::create_dir_all(sync_db.parent().unwrap()).unwrap();
...
async fn test_corruption_triggered_quarantine_blocks_cross_surface() {
    ...
    // Corrupt the merged gateway sync file; legacy sync.sqlite is not served.
    let replica_dir = env._tmp.path().join("users").join(&env.user_a.user_id);
    let sync_db = replica_dir.join("merged").join("sync.sqlite");
    if sync_db.exists() {
        corrupt_file(&sync_db);
    } else {
        // If the merged sync DB doesn't exist yet, create it as corrupt
        std::fs::create_dir_all(sync_db.parent().unwrap()).unwrap();

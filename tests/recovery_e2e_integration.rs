...
async fn test_running_server_restore_reports_rebuildable_and_device_recovers() {
    ...
    let sync_db = user_dir.join("merged").join("sync.sqlite");
    ...
    assert!(stdout.contains("Merged sync DB:       false"), "{stdout}");
    ...
    assert!(
        user_dir.join("merged").join("sync.sqlite").exists(),
        "merged sync DB should exist after the device recovery path runs"
    );
}
...
async fn test_mixed_surface_recovery_blocks_and_restores_rest_and_tc() {
    ...
    assert!(
        status == axum::http::StatusCode::OK || status == axum::http::StatusCode::NOT_FOUND,
        "restored gateway sync should be available or empty after backup restore, got {status}"
    );
}

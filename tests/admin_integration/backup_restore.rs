...
async fn create_admin_backup(env: &TestEnv) -> String {
    ...
    create.json::<Value>()["timestamp"]
        .as_str()
        .unwrap()
        .to_string()
}

fn refresh_manifest_config_db_file_entry(snapshot_dir: &Path, manifest: &mut Value) {
    let old_config_size = manifest["contents"]["config_db"]["size_bytes"]
        .as_u64()
        .unwrap_or_default();
    let config_bytes = std::fs::read(snapshot_dir.join("config.sqlite")).unwrap();
    let new_config_size = config_bytes.len() as u64;
    manifest["contents"]["config_db"]["size_bytes"] =
        Value::Number(serde_json::Number::from(new_config_size));
    let digest = {
    ...
    if let Some(total_size) = manifest["total_size_bytes"].as_u64() {
        manifest["total_size_bytes"] = Value::Number(serde_json::Number::from(
            total_size.saturating_sub(old_config_size) + new_config_size,
        ));
    }
}
...
async fn test_admin_backup_create_and_list_ignore_incomplete_snapshot() {
    ...
    let personal_scope = env
        ...
        .unwrap()
        .unwrap();
    let manifest_scope = &manifest["contents"]["replicas"][0]["task_scope"];
    assert_eq!(manifest_scope["task_scope_id"], personal_scope.id);
    assert_eq!(manifest_scope["key_prefix"], personal_scope.key_prefix);
    assert_eq!(manifest_scope["kind"], "personal");
    let replica_entry = &manifest["contents"]["replicas"][0];
    let canonical_file = replica_entry["canonical_task_db"]["file"].as_str().unwrap();
    assert!(
        canonical_file.ends_with("/taskchampion.sqlite3"),
        "backup should name the canonical Task Scope DB, got {canonical_file}"
    );
    assert_eq!(
        replica_entry["canonical_task_db"]["file"],
        replica_entry["replica_db"]["file"]
    );
    assert_eq!(
        replica_entry["canonical_task_db"]["sha256"],
        replica_entry["replica_db"]["sha256"]
    );
    assert_eq!(
        replica_entry["canonical_task_db"]["size_bytes"],
        replica_entry["replica_db"]["size_bytes"]
    );
...
#[tokio::test]
async fn test_admin_backup_restore_preserves_merged_sync_storage() {
    let env = setup().await;
    let user_dir = env._tmp.path().join("users").join(&env.user_id);
    let merged_sync_path = user_dir.join("merged").join("sync.sqlite");

    let storage = cmdock_server::tc_sync::storage::SyncStorage::open_merged(&user_dir).unwrap();
    let version_id = storage
        .add_version(Uuid::nil(), b"merged-sync-backup-fixture")
        .unwrap()
        .unwrap();
    ...
    let manifest: Value =
        serde_json::from_slice(&std::fs::read(backup_manifest_path(&env, &timestamp)).unwrap())
            .unwrap();
    let sync_file = manifest["contents"]["replicas"][0]["sync_db"]["file"]
        .as_str()
        .unwrap();
    assert!(
        sync_file.ends_with("/merged/sync.sqlite"),
        "backup should capture the gateway-served merged sync DB, got {sync_file}"
    );
    assert_eq!(
        manifest["contents"]["replicas"][0]["merged_sync_db"]["file"],
        sync_file
    );
    assert_eq!(
        manifest["contents"]["replicas"][0]["merged_sync_db"]["sha256"],
        manifest["contents"]["replicas"][0]["sync_db"]["sha256"]
    );
    assert_eq!(
        manifest["contents"]["replicas"][0]["merged_sync_db"]["size_bytes"],
        manifest["contents"]["replicas"][0]["sync_db"]["size_bytes"]
    );

    drop(storage);
    std::fs::remove_dir_all(user_dir.join("merged")).unwrap();
    assert!(!merged_sync_path.exists());
    ...
    assert!(
        merged_sync_path.exists(),
        "restore should put merged sync DB back at the runtime path"
    );
    let restored = cmdock_server::tc_sync::storage::SyncStorage::open_merged(&user_dir).unwrap();
    assert!(restored.version_exists(version_id).unwrap());
}

#[tokio::test]
async fn test_admin_backup_restore_rejects_task_scope_metadata_mismatch() {
    let env = setup().await;
    let (admin_h, admin_v) = auth_header(&env.admin_token);

    for (field, value) in [
        ("key_prefix", Value::String("WRONG".to_string())),
        ("task_scope_id", Value::String(Uuid::new_v4().to_string())),
        ("kind", Value::String("team".to_string())),
    ] {
        let timestamp = create_admin_backup(&env).await;
        let manifest_path = backup_manifest_path(&env, &timestamp);
        let mut manifest: Value =
            serde_json::from_slice(&std::fs::read(&manifest_path).unwrap()).unwrap();
        manifest["contents"]["replicas"][0]["task_scope"][field] = value;
        std::fs::write(
            &manifest_path,
        ...
        let restore = env
            ...
            .json(&serde_json::json!({ "timestamp": timestamp }))
            .await;
        restore.assert_status(axum::http::StatusCode::UNPROCESSABLE_ENTITY);
...
#[tokio::test]
async fn test_admin_backup_restore_rejects_inactive_task_scope_metadata() {
    let env = setup().await;
    let timestamp = create_admin_backup(&env).await;
    ...
    let conn = rusqlite::Connection::open(snapshot_dir.join("config.sqlite")).unwrap();
    conn.execute(
        "UPDATE task_scopes SET status = 'disabled' WHERE owner_runtime_user_id = ?1",
        [&env.user_id],
    )
    .unwrap();
    drop(conn);
    refresh_manifest_config_db_file_entry(&snapshot_dir, &mut manifest);
    std::fs::write(
        &manifest_path,
        serde_json::to_vec_pretty(&manifest).unwrap(),
    )
...
#[tokio::test]
async fn test_admin_backup_restore_accepts_pre_task_scope_manifest_without_task_scopes_table() {
    let env = setup().await;
    let timestamp = create_admin_backup(&env).await;
    let snapshot_dir = backup_root(&env).join(&timestamp);
    let manifest_path = snapshot_dir.join("manifest.json");
    ...
    let replica = manifest["contents"]["replicas"][0].as_object_mut().unwrap();
    replica.remove("task_scope");
    replica.remove("canonical_task_db");
    replica.remove("merged_sync_db");
    let conn = rusqlite::Connection::open(snapshot_dir.join("config.sqlite")).unwrap();
    conn.execute_batch(
        "PRAGMA foreign_keys=OFF;
         DROP TABLE task_scopes;
         DELETE FROM _migrations WHERE name = '031_create_task_scopes.sql';",
    )
    .unwrap();
    drop(conn);
    refresh_manifest_config_db_file_entry(&snapshot_dir, &mut manifest);
    ...
    assert!(
        env.store
            ...
            .unwrap()
            .is_some(),
        "post-restore migrations should recreate the user's Personal Task Scope"
    );
}

#[tokio::test]
async fn test_admin_backup_restore_rejects_task_scope_manifest_without_task_scopes_table() {
    let env = setup().await;
    let timestamp = create_admin_backup(&env).await;
    let snapshot_dir = backup_root(&env).join(&timestamp);
    let manifest_path = snapshot_dir.join("manifest.json");
...
async fn test_admin_backup_restore_accepts_older_compatible_manifest() {
    ...
    // Simulate a pre-Task-Scope manifest. The config DB still contains the
    // modern table because this test mutates only manifest metadata; restore
    // must continue accepting older manifest shapes.
    let replica = manifest["contents"]["replicas"][0].as_object_mut().unwrap();
    replica.remove("task_scope");
    replica.remove("canonical_task_db");
    replica.remove("merged_sync_db");
...
async fn test_admin_backup_restore_tc_sync_updates_admin_last_sync() {
    ...
    let create_device = env
        .server
        .post(&format!("/admin/user/{}/devices", env.user_id))
        .add_header(admin_h.clone(), admin_v.clone())
        .json(&serde_json::json!({
            "name": "Restore Sync Device",
        }))
        .await;
    create_device.assert_status(axum::http::StatusCode::CREATED);
    let client_id = create_device.json::<Value>()["clientId"]
        .as_str()
        .unwrap()
        .to_string();

...
### Config-DB `synchronous=NORMAL` is NOT crash-safe (cross-DB durability gap)

**Verdict: NO-GO** for `PRAGMA synchronous=NORMAL` on the `config.sqlite`
connection (proposed under release issue #151 / parent #146 as a write-path perf
win). This was the output of the #150 crash-recovery matrix spike; the verdict was
independently confirmed by a codex review.

**Why.** A `add_task` mutation spans two independent SQLite databases:

- `config.sqlite` — the `task_key_allocations` table (`pending|committed|burned`
  rows; the authoritative source for the human key `PREFIX-N` and for the wire
  `TaskItem.key` projection).
- the per-user TaskChampion replica — the task itself plus its `cmdock_key` UDA.

Today **both** run WAL + `synchronous=FULL` (config sets only `journal_mode=WAL`,
so `synchronous` defaults to FULL; TaskChampion 3.0.1 likewise sets only WAL).
Both are therefore durable per-commit. `synchronous=NORMAL` makes config commits
durable only at the next checkpoint — on **power loss / OS crash / hard reset**
(not application crash) SQLite can roll back the entire un-checkpointed WAL
suffix. Because #151 leaves the TC replica at FULL, it makes config the *only*
loseable side while the task + `cmdock_key` UDA survive.

**Crash matrix:**

| State | config side | TC side | Recovered? |
|---|---|---|---|
| 1. reserve lost, TC task not committed | row gone | no task | ✅ consistent — no task ever used N |
| 2. attached-pending lost, TC committed | row gone | task + UDA | ❌ see gap below |
| 3. **committed allocation lost, TC committed** | row gone | task + UDA | ❌ **the gap** |
| 4. idempotency dedup record lost | row gone | task + allocation OK | ⚠️ retried key re-executes → possible dup task; widens an already-documented residual window, not a new bug class |

States 2 and 3 both reduce to *allocation row gone, TC task + `cmdock_key` UDA
present*, and **no existing mechanism reconstructs a lost committed (or fully
lost) allocation row for a migrated user**:

- the reaper (`src/task_keys/reaper.rs`) only scans `state='pending'` rows;
- backfill (`src/task_keys/backfill.rs`) returns early once
  `users.task_keys_migrated_at IS NOT NULL`;
- drift recovery (`src/task_keys/drift.rs`) explicitly SKIPs no-allocation-row
  tasks (`task_keys_drift_skipped_no_row_total`, observed-only).

Consequences: (a) `TaskItem.key` projects `null` for an existing keyed task; and
worse (b) a **silent cross-DB duplicate key** — if the lost row held `MAX(n)`,
the next `reserve` recomputes `MAX(n)+1` over surviving rows and re-issues the
same N to a different new task, while the surviving task still carries
`cmdock_key=PREFIX-N`. No DB constraint catches this (the old row is gone; the
new row has a different `task_uuid`). Because NORMAL loses the whole
un-checkpointed suffix on the single shared config connection, a busy-moment
power crash can drop **many** committed rows across **many** users at once — a
multi-task hazard, not a single edge case. This violates the contract's
"keys are unique / burned numbers never re-allocate."

**Cost/benefit makes the NO-GO decisive, not marginal.** The #152 measurement
showed `tx.commit()` fsync is flat at ~5ms (1.1× across 1→50 users) and is only
~6% of the `reserve` call at 50 users; the bottleneck is queue-wait on the single
connection (443×), which #147 (read pool) removes. #151's entire benefit is
removing a ~5ms flat fsync that is not the bottleneck.

**If ever revisited**, `synchronous=NORMAL` MUST be preceded by a startup
cross-DB integrity reconciler that, *before serving traffic*, scans every user's
TC tasks for `cmdock_key` UDAs lacking a committed allocation row and reconstructs
them (extending backfill's orphan/matched reconciliation to run for ALL users,
including already-migrated). Note the orphan rule forbids adopting a foreign N, so
a surviving task may be **re-keyed** (client-visible). The clean alternative is to
keep config allocation writes at FULL; `wal_autocheckpoint` tuning or per-write
checkpoints only shrink the window or re-introduce the sync cost. See release
issue #150 (matrix) and the recovery-gap follow-up it filed.

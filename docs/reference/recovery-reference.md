...
## 13.1 Per-Account-Lazy Task-Keys Backfill (#130 Phase 4)

Existing tasks (created before #130 shipped) need allocation rows in
`task_key_allocations` and a `cmdock_key` UDA on the TC task before the
wire `TaskItem.key` field can be populated. The backfill is
**per-account-lazy**: triggered on the first server access for each user
after Phase 4 ships, runs synchronously under the per-user mutation
lock (the same lock `service::add_task` and the reaper take), and
completes in one request per user lifetime.

Recovery walks for this migration:

- A crash **before** the atomic Phase A+C commit leaves zero allocation
  rows in the config DB. The next call replays Phase B (idempotent —
  matching UDA values are skipped) and Phase A+C from a clean slate. A
  `task.key.migration_recovery` audit event fires when at least one
  pre-existing UDA value matches what the backfill would write,
  signalling the resume.
- A crash **after** the atomic Phase A+C commit populates
  `users.task_keys_migrated_at`. The next call observes that on the
  post-lock double-check and returns immediately, marking the in-memory
  cache.
- Cache invalidation goes through `RuntimeRecoveryCoordinator::evict_user`,
  the single shared owner of the per-user runtime-cache eviction recipe.
  Restore (`OperatorMaintenanceBackend::restore`), `delete_user`, and
  offline-quarantine all funnel through `evict_user`, so a checkpoint
  restore that rolls back `task_keys_migrated_at` to NULL invalidates
  the cache automatically and the next access re-runs the backfill.

The backfill is **synchronous on first access** — wire `TaskItem.key`
is non-null per the contract's wire-vs-cache nullability rule. For users
with very large task counts, first-access latency can be noticeably
higher than subsequent calls; the per-user mutation lock is held for
the duration, blocking that user's mutations and reaper turns.

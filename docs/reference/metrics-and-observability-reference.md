...
- `request_id`
  - populated from `X-Request-ID` on every request. The `request_id`
    middleware (`src/request_id.rs`) guarantees the header is always
    present: if the client does not supply it, a fresh UUID v4 is generated
    and injected into both the request (visible to all downstream handlers
    and audit events) and the response. Clients may supply their own value
...
## 6a. MergedSyncGateway signals (server#143)

The merged TaskChampion gateway emits aggregate metrics for the beta-blocking
observability gate:

- `merged_sync_gateway_codec_failures_total{stage}` — private history-segment
  decode failures. A non-zero rate after a TaskChampion upgrade usually means
  the codec corpus/review gate missed a wire-shape change.
- `merged_sync_gateway_journal_transitions_total{from,to}` and
  `merged_sync_gateway_journal_state_entries_total{state}` — forward-only
  journal movement through `received`, `merged_version_accepted`,
  `source_plan_applied`, `projection_appended`, `finalized`, `failed`, and
  `quarantined`.
- `merged_sync_gateway_recovery_outcomes_total{outcome}` — terminal recovery
  status for finalized/failed/quarantined journal attempts.
- `merged_sync_gateway_projection_duration_seconds{result,changed}` and
  `merged_sync_gateway_projection_runs_total{result,changed}` — source-to-
  merged projection latency and outcome.
- `merged_sync_gateway_snapshot_versions_since_snapshot` and
  `merged_sync_gateway_snapshot_urgency_total{level}` — snapshot age/urgency
  input used by `add-version` responses (`none`, `low`, `high`).
- `merged_sync_gateway_retention_outcomes_total{kind,result}` — stale-client
  `410 Gone` and retention-GC outcomes.

Per-user/journal attribution is intentionally not encoded as metric labels.
Use audit logs and `/admin/user/{user_id}/stats` for high-cardinality
questions such as which client/version caused a write.
...
## 7a. Task-keys observability (#130)

The task-keys subsystem (`<PREFIX>-N` allocation, `cmdock_key` UDA on TC,
backfill, drift recovery, reaper) emits a small family of counters and
audit lines tracked together because operators read them as one
ensemble. The signals split by lifecycle:

- **Steady-state allocation** (`task_keys_allocated_total`,
  `task_keys_burned_total{reason}`) — answers "are creates landing
  cleanly, and how often does Phase 3 burn-on-failure fire?".
- **Backfill — first per-account run.**
  `task_keys_migration_started_total` /
  `task_keys_migration_completed_total` bracket each lazy-on-first-
  access pass per `task-write-contract.md` § Migration of existing tasks.
  `task_keys_migration_recovery_total{kind}` distinguishes recovery
  causes: `phase_b_uda` (prior crashed Phase B; #141 may also stamp the
  missing `cmdock_account` UDA in this path without adding a new metric
  kind), `pending_attached` (`reconcile_pending_attached_rows`),
  `orphan_reconciled` (Phase 5e foreign-UDA reconciliation — task
  carried a `cmdock_key` value with no allocation row, fresh-N allocated
  and the foreign value overwritten). Normal first-time backfill that
  writes `cmdock_key` + `cmdock_account` for existing tasks only affects
  `task_keys_migration_started_total` / `completed_total`; it is not
  reported as orphan growth. The dedicated counter
  `task_keys_orphans_reconciled_total` answers "how many orphan tasks
  did this server's backfill see this week?" without needing label
  arithmetic.
- **Sync-bridge drift recovery** (Phase 5b — `task_keys_drift_recovered_total{kind}`,
  with `kind ∈ {value_mismatch, post_commit_finalize, pending_with_drift}`)
  — fires when the sync-bridge observes a `cmdock_key` UDA op in the
  canonical replica that disagrees with a non-burned allocation row.
  Paired with the operational counter
  `task_keys_drift_skipped_no_row_total` for the "task carries
  `cmdock_key` UDA but no allocation row" case (per contract: drift
  recovery does NOT run on no-row tasks; Phase 5e backfill picks them
  up on the next pass — the metric tracks frequency for operator
  visibility, not contract). The server also emits
  `task_keys_account_only_drift_observed_total` when `cmdock_key`
  still matches the allocation row but canonical `cmdock_task_scope` or
  deprecated `cmdock_account` is missing or different. That counter is
  observability-only for the sync-bridge drift path: matching-key
  scope/account drift is observed rather than repaired inline, while the
  first-access upgrade/backfill path stamps both aliases from allocation
  rows.
- **Reaper turns** (`task_keys_reaper_skipped_total{reason}` with
  `reason ∈ {replica_open_busy, replica_busy, uda_mismatch}`) — answers
  "is the reaper making progress, or is it skipping users due to lock
  contention?". The `uda_mismatch` reason is intentionally not
  escalated to burn-with-UDA-clear (see CLAUDE.md § Reaper
  SkipUdaMismatch); operators investigate manually.

Audit-side counterpart: actions all share the `task.key.*` namespace
(`task.key.migration_started`, `task.key.migration_completed`,
`task.key.migration_recovery`, `task.key.drift_recovered`,
`task.key.reaper_uda_mismatch`). Per-task detail lives in audit;
aggregate trends live in the counters.

Boundary rule (echoes § 4): if you see a single failed migration in
audit, that's an operator follow-up; if `task_keys_drift_skipped_no_row_total`
is climbing fast across many users, that's a backfill scheduling issue
worth alerting on. If `task_keys_account_only_drift_observed_total`
rises, the sync bridge is seeing pre-upgrade or externally-clobbered
Task Scope/account UDA state that is intentionally observed-but-not-
repaired by the sync-bridge drift path.

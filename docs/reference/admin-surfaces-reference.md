...
`/admin/user/{id}/stats` includes per-user merged-sync gateway diagnostics under
`mergedSync`: journal state counts and the 20 most recently updated journal
attempts with `journalId`, `gatewayAttemptId`, `clientId`, parent/merged version
IDs, state, recovery status, diagnostics, and timestamps. This is the operator
surface for answering which TW version caused a source write and whether its
forward recovery finalized, failed, or quarantined.
...
- backend-specific file-level maintenance (checkpoint, file backup,
  file restore) lives behind the `OperatorMaintenanceBackend` trait
  (`src/store/maintenance.rs`), separate from `ConfigStore`. The trait
  is implemented by `SqliteConfigStore` today; a future Postgres
  backend would supply its own handle (e.g. `pg_basebackup` wrapper)
  or reject these calls entirely. See ADR-0002 § component boundaries.
...
- operator HTTP device lifecycle
  - list/create/show/rename/revoke/unrevoke/rotate/delete on target-user gateway-bound TW identities
  - `POST /admin/user/{id}/devices/{id}/rotate` is the rotation-recovery path:
    atomically revokes the old device and provisions a replacement. The old
    `client_id` credential is rejected by TC sync immediately (403 FORBIDDEN);
    the new credential authenticates on the first request. The `device.rotate`
    audit entry carries both `old_client_id` and `client_id` (new) with
    `reason = "rotation_recovery"` so `cmdock-admin doctor` can reconstruct the
    identity transition from a single audit record.

That mirrors the runtime model:

- one canonical sync identity per user (server-side translation material)
- one gateway-bound device record per physical client (`client_id` + per-device encrypted secret + personal source-account binding)
...
- device provisioning, rotation, and device lifecycle now use the shared device service
...
- device list/show responses expose `syncRuntime=mergedGateway` and the personal `sourceAccount` so self-host and managed issuers can verify they minted the same runtime identity shape
...
## 9a. Task-key prefix administration (#130)

Per `task-write-contract.md` § Task Keys, every user has a server-wide-unique
task-key prefix (`^[A-Z][A-Z0-9]{0,9}$`) that prefixes every `<PREFIX>-N` key.
Two CLI surfaces:

- **`cmdock-server admin user create --username <name> [--prefix <PREFIX>]`** —
  if `--prefix` is omitted, the server derives one from the username via the
  canonical algorithm (ASCII-filter → uppercase → first-char ensure → 10-char
  truncate → suffix-on-collision → UUID-hex fallback). Same algorithm cmdock-admin
  / iOS / obsidian use client-side, so derivations are byte-identical.
- **`cmdock-server admin user set-prefix <user_id> <PREFIX>`** — operator override.
  Useful for collision resolution or operator preference before any tasks have
  been allocated under the current prefix.

### Prefix immutability

`set-prefix` rejects with `PREFIX_LOCKED` if **any** allocation row exists for
the user (any state — pending, committed, OR burned). Rationale: changing the
prefix after keys have been allocated would orphan the (prefix, n) → UUID
mapping and break clients holding cached keys. A future "rebrand" use case
would need a separate command with explicit multi-step migration semantics;
not in scope today.

### Startup backfill

On every boot, the server walks users with NULL prefix and assigns derived
prefixes. Idempotent — a second boot finds no NULL rows and exits in O(1).
Backfill failure is logged as a warning and does not block startup; operators
can recover with `admin user set-prefix`.

### Failure modes

- `INVALID_PREFIX: <reason>` — supplied prefix doesn't match `^[A-Z][A-Z0-9]{0,9}$`.
- `PREFIX_TAKEN: prefix '<X>' is already used by another user` — server-wide
  uniqueness collision.
- `PREFIX_LOCKED: cannot change prefix after task allocations exist` — see
  immutability above.

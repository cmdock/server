...
### Wire-body conventions

Error responses never return JSON. Per the contracts in `cmdock/architecture`
(`task-write-contract.md` § Error vocabulary, `task-read-contract.md` §
Wire-body convention) the established conventions are:

- **400** — plain-text bare error code in the body. Examples: `INVALID_FIELD`,
  `INVALID_DATE`, `INVALID_RAW`, `INVALID_CONTEXT_ID`, `INVALID_QUERY_PARAM`,
  `INVALID_UUID`, `TOO_MANY_UUIDS`, `EMPTY_UUIDS`. No JSON envelope, no
  structured fields. Status code is the structured layer.
- **401** — plain-text short message (`Invalid token`, `Missing Authorization
  header`).
- **404** — **empty body** (`Content-Length: 0`, no `Content-Type`). This
  convention is iOS-app-load-bearing and predates the contracts; it is
  preserved across all endpoints. The empty body also enforces the
  existence-leak rule on `GET /api/tasks/{uuid}` automatically — there is no
  body content that could leak existence by varying.
- **500** — plain-text short message.

The "Code" column in contract error tables is a documentation label for the
failure condition, not necessarily the wire body. Code-bearing 400 bodies
match the label exactly; code-bearing 404 bodies do not exist.
...
- `GET /api/tasks` (three valid request shapes — see below)
- `GET /api/tasks/{uuid}` — singleton lookup (added in #109, governed by
  `task-read-contract.md` in `cmdock/architecture`)
- `POST /api/tasks`
...
`GET /api/tasks` request shapes (per the task-read contract, *§ GET /api/tasks
request shapes*):

| Shape | Body | Owning contract |
|-------|------|-----------------|
| `GET /api/tasks` (no params) | `Vec<TaskItem>` — pending list, UUID-asc | `default-views-contract.md` |
| `GET /api/tasks?view=<id>` | `Vec<TaskItem>` — view-scoped, UUID-asc | `default-views-contract.md` |
| `GET /api/tasks?uuids=<csv>` | `{found, missing}` — request-order preserved | `task-read-contract.md` |

`view` and `uuids` are mutually exclusive. The endpoint is **strict-recognise**
on query parameters: unknown keys, repeated keys, and `view`+`uuids` together
all return `400 INVALID_QUERY_PARAM` (plain-text bare code).

`GET /api/tasks/{uuid}` returns the task at any status (pending, completed, or
deleted) per the contract's "have UUID, can read state" visibility rule.

**Path-parameter resolution (`#130` Phase 3).** All five task-path endpoints
(`GET /api/tasks/{uuid}` and `POST /api/tasks/{uuid}/{modify,done,undo,delete}`)
accept either a canonical UUID **or** a task key in the form `<PREFIX>-N`
(prefix `^[A-Za-z][A-Za-z0-9]{0,9}$`, N `^[1-9][0-9]*$`). Prefix is
case-insensitive on input — `WORK-15` and `work-15` resolve to the same task.
UUID strictness is **asymmetric** by endpoint, intentionally:

| Endpoint                              | UUID branch                                      |
|---------------------------------------|--------------------------------------------------|
| `GET /api/tasks/{uuid}`               | Canonical-only (lowercase hyphenated, per #109)  |
| `POST /api/tasks/{uuid}/modify`       | Permissive (`Uuid::parse_str` — uppercase/etc OK)|
| `POST /api/tasks/{uuid}/done`         | Permissive                                       |
| `POST /api/tasks/{uuid}/undo`         | Permissive                                       |
| `POST /api/tasks/{uuid}/delete`       | Permissive                                       |

The asymmetry preserves existing client behaviour — tightening mutation
endpoints to canonical-only is a separate gated follow-up requiring contract
sign-off + iOS audit (Decisions Locked In iter3 of #130). Unknown UUIDs,
unknown keys, and cross-account identifiers all return `404` with empty body
(existence-leak rule — indistinguishable per byte parity). Malformed input
(neither UUID nor syntactically valid key) returns `400` — `INVALID_UUID`
plain-text body on the read singleton, empty body on mutation endpoints
(legacy iOS compatibility).

`GET /api/tasks?uuids=<csv>` validation pipeline (normative ordering):

1. Parse CSV.
2. `400 EMPTY_UUIDS` if empty after parse (`?uuids=`).
3. `400 TOO_MANY_UUIDS` if `len(raw_entries) > task_read.batch_max_uuids`
   (default 100, server-configurable). **Cap applied before deduplication** —
   101 copies of one UUID still trips the cap.
4. `400 INVALID_UUID` for any malformed entry, including empty CSV segments
   from leading/trailing/consecutive commas. The offending entry's index is
   recorded in the audit log (action `task.read.batch.invalid_uuid`,
   `invalid_index` field) and tracing — **not** in the wire body.
5. Deduplicate, preserving first-occurrence order.
6. Resolve each unique UUID against the per-user replica.

Response: `{found: TaskItem[], missing: string[]}`. Both arrays preserve
request-order (not UUID-sorted) — the dashboard use case depends on the
client's curated order. `found.length + missing.length` always equals the
deduped request count.

Default list semantics:

- omitting `view` and `uuids` returns the normal pending-task list
- sending `view=` with an empty value is treated the same as omitting `view`
- deleted tasks are not part of the no-params or view-scoped lists; they ARE
  visible via singleton and batch lookup
- each task item includes computed `blocked` and `waiting` booleans
- each task item includes `depends: [uuid]` — sorted list of pending dependency
  UUIDs. On list and summary responses, `blocked == (depends.length > 0)`.
  On mutation webhook payloads (create, complete, delete, undo), `depends` may
  be empty even when `blocked` is true — treat `blocked` as authoritative.
  Available as a webhook `modified_fields` filter value (`"depends"`)
- user-defined attributes (UDAs) are emitted as top-level string keys on each
  task item, matching the Taskwarrior JSON format. Any TaskChampion property not
  consumed by the explicit schema (e.g. `estimate`, `energy`) passes through
  automatically. Values are always strings. Individual UDA names are valid
  webhook `modified_fields` filter values
- `cmdock_task_scope` is the canonical Task Scope prefix projection for TW
  interoperability (TSKEY-007/SG-011). `cmdock_account` is no longer emitted in
  `TaskItem` responses; it is suppressed server-side and filtered from the REST
  projection. Clients may still send `cmdock_account` in request bodies — it is
  accepted without error and silently ignored. Only `cmdock_task_scope` is
  validated against the user's prefix (`400 INVALID_TASK_SCOPE` on mismatch).
...
**`TaskItem.key` nullability (`#130` Phase 5d).** `TaskItem.key` is a
**nullable** top-level string (`string | null`) per the task-write contract,
*§ Wire exposure*.
The field is always present on the wire (never omitted) — REST emits explicit
`null` whenever there is no `committed` row and no non-expired `pending` row in
the allocation table for the task's UUID. Four transient causes:

1. **Pre-migration / migration-in-retry.** No allocation row exists for the
   account's tasks until the per-account-lazy backfill commits.
2. **Burned.** Phase 3 finalize failed; the row is `burned` until the next
   backfill pass allocates a fresh row.
3. **Expired pending (lookup-time).** A `pending` row exists but its
   `created_at + pending_timeout < now`. REST evaluates expiry against the
   current time, decoupling correctness from reaper scheduling — the row
   projects as `null` even if the reaper has not yet physically transitioned
   it to `burned`. Mirrors the Idempotency Phase 1 pending-record lookup-time
   expiry behaviour in the task-write contract (same
   `task_write.idempotency_pending_timeout_seconds` config value).
4. **Orphan.** The task carries a `cmdock_key` UDA on the canonical replica
   but no `pending` or `committed` allocation row. Awaits the
   orphan-reconciliation pass.

All four resolve on the next backfill / reaper / orphan-reconcile pass.
Clients MUST handle `null` gracefully (display the UUID short-form, omit the
key column, etc.) but MUST NOT treat a null key as task non-existence.
...
Webhook delivery payloads include the logical `task_scope_id` for real task and
sync events, resolved from the user's Personal Task Scope. Synthetic
`webhook.test` deliveries do not carry a Task Scope. The server signs delivery
attempts with the configured shared secret, records delivery attempts, and
retries failed deliveries with bounded backoff. The delivery history endpoint is
the supported way to inspect recent success/failure for one webhook.
...
- `GET /admin/user/{id}/devices`
  - list gateway-bound TW identities for the target user, including bootstrap lifecycle state, `syncRuntime`, and personal `sourceAccount`
- `POST /admin/user/{id}/devices`
  - create a new gateway-bound per-device credential for the target user and return client-ready sync config
- `GET /admin/user/{id}/devices/{client_id}`
  - show a single gateway-bound device record for the target user
...
- `POST /admin/user/{id}/devices/{client_id}/rotate`
  - revoke the old client id and mint a replacement client id/secret with the same Taskwarrior `.taskrc` shape
...
identity, runtime-policy, and per-user gateway-bound device lifecycle surfaces with a separate

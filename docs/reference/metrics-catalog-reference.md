...
- Labels:
  - `call`
- Meaning:
  - caller-side wall-clock duration of individual `ConfigStore` calls on the
    task write path, measured around the `await` — i.e. queue-wait on the single
    shared `config.sqlite` connection **plus** query execution **plus** commit
- Current `call` values:
  - `get_user_prefix`
  - `reserve`
  - `commit_task_key`
...
- this is the primary signal for the #146 write-path serialization ceiling: all
  config reads and writes funnel through one `tokio_rusqlite::Connection`, so hot
  reads queue behind writes. The fsync-free `get_user_prefix` read grows with the
  concurrent user count (0.12ms → 44ms, 1 → 50 users) even though it contends on
  no replica lock — pure queue-wait, not lock contention or disk
- to estimate the queue-wait + non-commit execution component, subtract the
  **mean** of `config_store_commit_seconds` from the **mean** of
  `config_call_seconds` for the same `call`
  (`config_call_seconds_sum/_count − config_store_commit_seconds_sum/_count`).
  Subtract means, not quantiles — `p99(total) − p99(commit) ≠ p99(remainder)`

### `config_store_commit_seconds`

- Type: histogram
- Labels:
  - `call`
- Meaning:
  - inside-closure duration of `tx.commit()` for a config-DB write, recorded on
    the `tokio_rusqlite` background thread — isolates the fsync / write-exec cost
    from the queue-wait captured by `config_call_seconds`
- Current `call` values:
  - `reserve`

Operator note:

- a small `config_store_commit_seconds` alongside a large `config_call_seconds`
  for the same `call` confirms the bottleneck is queue-wait (single-connection
  serialization), not fsync. This is the measurement that gates the cost/benefit
  of `synchronous=NORMAL` (#151)
...
### `webhook_dispatch_in_flight`

- Type: gauge
- Labels: none
- Meaning:
  - number of webhook dispatches (target lookup + delivery) spawned off the
    synchronous mutation response path (#149) that have not yet completed
- Operator note:
  - delivery retries run inline (1s/10s/60s), so a slow/dead endpoint keeps a
    dispatch in flight for the full retry budget. Dispatch is bounded (#156) by a
    semaphore (default 512, `CMDOCK_WEBHOOK_DISPATCH_MAX_INFLIGHT`); this gauge
    approaching the cap means dispatch is backing up (slow/failing endpoints)
  - in-flight count is `capacity - available_permits`; at the cap, new events are
    shed (see `webhook_dispatch_shed_total`)
...
- Labels: none
- Meaning:
  - mutation-driven webhook events **dropped without delivery** because dispatch
    was at capacity when they were emitted (#156)
- Operator note:
  - a shed is a permanent drop (no retry). Any non-zero value means the dispatch
    cap was saturated — almost always a dead/slow endpoint holding permits for
    the full retry budget. Investigate the offending webhook (it auto-disables
    after 10 consecutive failures) and/or raise `CMDOCK_WEBHOOK_DISPATCH_MAX_INFLIGHT`
  - the cap is **global**, so one user's dead-hook storm can shed other users'
    events until the hook auto-disables (known limitation — see #156)
...
## 8a. Merged Sync Gateway Metrics

### `merged_sync_gateway_codec_failures_total`

- Type: counter
- Labels: `stage`
- Meaning: private TaskChampion history-segment codec failures.

### `merged_sync_gateway_journal_transitions_total`

- Type: counter
- Labels: `from`, `to`
- Meaning: forward-only gateway journal transitions.

### `merged_sync_gateway_journal_state_entries_total`

- Type: counter
- Labels: `state`
- Meaning: number of times attempts entered each gateway journal state.

### `merged_sync_gateway_recovery_outcomes_total`

- Type: counter
- Labels: `outcome`
- Meaning: terminal recovery/finalization outcomes for gateway attempts.

### `merged_sync_gateway_projection_duration_seconds`

- Type: histogram
- Labels: `result`, `changed`
- Meaning: latency of personal source-to-merged projection passes.

### `merged_sync_gateway_projection_runs_total`

- Type: counter
- Labels: `result`, `changed`
- Meaning: count of personal projection passes.

### `merged_sync_gateway_snapshot_versions_since_snapshot`

- Type: histogram
- Labels: none
- Meaning: merged-chain versions since latest snapshot when `add-version` evaluated snapshot urgency.

### `merged_sync_gateway_snapshot_urgency_total`

- Type: counter
- Labels: `level`
- Meaning: gateway snapshot urgency classifications (`none`, `low`, `high`).

### `merged_sync_gateway_retention_outcomes_total`

- Type: counter
- Labels: `kind`, `result`
- Meaning: stale-client and retention-GC outcomes.

## 8b. Task Key And Task Scope Metrics

Allocation, migration/backfill, drift recovery, and reaper telemetry for the
`cmdock_key` / Task Scope subsystem (server#130). See
[Metrics and Observability Reference §7a](metrics-and-observability-reference.md)
for interpretation and alerting guidance.
...
- Type: counter
- Labels: none
- Meaning: task keys allocated (create path + backfill).

### `task_keys_burned_total`

- Type: counter
- Labels: `reason` (`reaper`, `backfill_reconcile_orphan`)
- Meaning: allocation rows burned (permanent counter gaps); burned numbers never re-allocate.

### `task_keys_migration_started_total` / `task_keys_migration_completed_total`

- Type: counter
- Labels: none
- Meaning: bracket each lazy-on-first-access Phase 4 backfill migration for a user.

### `task_keys_migration_recovery_total`

- Type: counter
- Labels: `kind` (`phase_b_uda`, `pending_attached`)
- Meaning: recovery of a prior crashed/partial backfill pass.

### `task_keys_orphans_reconciled_total`

- Type: counter
- Labels: none
- Meaning: backfill reconciled a task carrying a foreign `cmdock_key` (fresh-N overwrite; never adopts the foreign value).

### `task_keys_drift_recovered_total`

- Type: counter
- Labels: `kind`
- Meaning: legacy sync-bridge (Phase 5b) reverse-UDA drift recovery on the canonical replica. The merged gateway path uses the `merged_sync.*_corrected` audit events instead.

### `task_keys_drift_skipped_no_row_total`

- Type: counter
- Labels: none
- Meaning: drift recovery saw a `cmdock_key`-bearing task with no allocation row and skipped it (backfill picks it up on the next pass).

### `task_keys_account_only_drift_observed_total`

- Type: counter
- Labels: none
- Meaning: a task carries `cmdock_account` without a paired `cmdock_key` drift — observe-only signal; no repair action until a contract amendment (TSKEY-007/SG-011).

### `task_keys_reaper_skipped_total`

- Type: counter
- Labels: `reason` (`replica_open_busy`, `replica_busy`, `uda_mismatch`)
- Meaning: reaper skipped a user/row this pass (contention or UDA mismatch left for operator review).

### `task_keys_reaper_finalised_total`

- Type: counter
- Labels: none
- Meaning: stale pending allocation rows the reaper finalised (committed or burned).

### `task_keys_reaper_uda_cleared_total`

- Type: counter
- Labels: none
- Meaning: reaper burn-with-UDA-clear operations (reverse `cmdock_key` op emitted before burn).

### `task_keys_reaper_pass_seconds`

- Type: histogram
- Labels: none
- Meaning: wall-clock duration of a full reaper pass.

### `task_keys_reaper_lock_acquire_seconds`

- Type: histogram
- Labels: none
- Meaning: per-user mutation-lock acquisition latency inside the reaper.

### `task_keys_prefix_backfill_errors_total`

- Type: counter
- Labels: none
- Meaning: non-fatal failures during boot-time user-prefix backfill (operator runs `admin user set-prefix` to recover).

### `task_scope_ensure_total`

- Type: counter
- Labels: none
- Meaning: Task Scope ensure operations (scope rows created/confirmed for a user).

### `task_scope_backfill_errors_total`

- Type: counter
- Labels: none
- Meaning: non-fatal failures during Task Scope backfill.

### `task_key_allocations_missing_task_scope_id`

- Type: gauge
- Labels: none
- Meaning: count of committed allocation rows still missing a `task_scope_id` (set at boot; `0` is healthy steady state after the scope-routing migration).

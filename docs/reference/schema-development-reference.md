...
- gateway-served per-user `merged/sync.sqlite`
  - repo-owned code-driven open-time uplift
  - `metadata.schema_version` is advisory and diagnostic
  - older `sync.sqlite` files outside `merged/` are legacy/non-serving artifacts
...
## 6. Gateway `merged/sync.sqlite` Rules

When changing the gateway-served sync storage:

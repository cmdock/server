...
The server also assigns a task-key prefix derived from the username (e.g.
`alice` → `ALICE`); pass `--prefix WORK` to override. The prefix is shown in
the create output and prefixes every `<PREFIX>-N` task key issued to the user.

**Upgrading from an older version:** existing users get derived prefixes
automatically on the next server boot (idempotent post-migration backfill).
Operators who want to rename a derived prefix BEFORE any tasks are allocated
can use `cmdock-server admin user set-prefix <user_id> <PREFIX>`. After the
first task has been allocated, `set-prefix` rejects with `PREFIX_LOCKED`.

**First-access task-keys backfill latency.** Pre-existing tasks (created
before the task-keys feature shipped) are migrated lazily on each user's
first server access after upgrade — the user's first request opens the
TC replica, writes the `cmdock_key` UDA on every existing task in one
batch, and inserts allocation rows in one config-DB transaction.
First-access latency scales with task count; for users with thousands
of tasks the extra round-trip can be hundreds of milliseconds. Subsequent
requests hit a cache and pay no extra cost. The migration runs once per
user lifetime; if you'd prefer to amortise it before users connect,
issue a `GET /api/app-config` request per user against a freshly
upgraded server.
...
| `users/` | **Yes** | Task data. One canonical replica plus merged-gateway sync state. |
...
| `CMDOCK_TASK_READ_BATCH_MAX_UUIDS` | Hard cap on raw entries in `GET /api/tasks?uuids=<csv>`. Applied **before** deduplication per `task-read-contract.md` § Limits. Non-positive or non-numeric values are ignored (TOML/default value retained). | `100` |

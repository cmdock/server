...
- `data/users/<user-id>/merged/sync.sqlite`
- matching `-wal` and `-shm` files for the merged gateway sync DB if present
- `data/users/<user-id>/merged/replica/taskchampion.sqlite3` and sidecars if present
- optional legacy `data/users/<user-id>/sync.sqlite` or `sync/<client-id>.sqlite` files if they exist in your environment
- your `config.toml`
...
then the server can usually reconstruct a *working* gateway TaskChampion sync DB
for that user by projecting canonical state back into a fresh `merged/sync.sqlite`.
...
- logical recovery of a usable gateway sync DB

not:

- exact restoration of the original sync database

## 2.2 Why This Is Still Not a Substitute for Backing Up Gateway Sync DBs
...
Webhook signing secrets and `--include-secrets`:

- **Plaintext webhook signing secrets are never written to the manifest** (this
  is an explicit architecture prohibition).
- Webhook signing capability is instead carried by the **encrypted** secret rows
  inside the backed-up `config.sqlite` (`webhooks.secret_enc`,
  `admin_webhooks.secret_enc`). The manifest records this with
  `secrets.webhook_signing_via_encrypted_config_db: true`.
- Those encrypted rows are only usable if the restore target is started with the
  **same `master_key`** that was in use when the webhooks were registered. If you
  restore onto a host with a different master key, webhook signing must be
  reconfigured even though the snapshot "included secrets". Keep the master key
  with — but not inside — your backups.
...
Before a live change, take the cold snapshot using your deployment's snapshot tooling. For self-hosters, the equivalent can be a `tar`, filesystem snapshot, VM snapshot, or image-level backup taken with the service stopped.
...
- are the canonical replica and the merged gateway sync DB logically consistent enough to resume service safely?
...
- the merged-gateway TaskChampion sync DB

An ideal backup/restore gives you both from the same logical point in time.

That is the cleanest case because:

- canonical state matches the protocol-facing merged sync state as of the same backup point
- the gateway has minimal projection/recovery work to do
...
- canonical `taskchampion.sqlite3` is newer than `merged/sync.sqlite`
- `merged/sync.sqlite` is restored only partially
...
- letting the merged gateway project canonical state into stale gateway sync state
- rebuilding a missing or unusable `merged/sync.sqlite` from canonical state
...
- if the gateway sync DB is missing or obviously behind, allow rebuild from canonical if the device identities are still trusted
...
but `merged/sync.sqlite` is missing, recovery is still often possible.

For an active device with intact registry metadata, the practical options are:

1. Let the server rebuild a fresh merged sync DB from canonical state.
...
They do not remove the gateway `merged/sync.sqlite`.

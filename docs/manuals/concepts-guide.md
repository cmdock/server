...
- **Replica**
  In general, a TaskChampion task database. In this server there are two important kinds: the canonical per-user replica and the merged-gateway projection replica/cache.
...
- **MergedSyncGateway**
  The runtime orchestrator that accepts TaskChampion sync operations, applies authorized source writes to canonical task state, and projects/corrects them into the durable merged sync chain.

- **Sync bridge**
  Legacy/internal bridge machinery that still supplies some lower-level projection and scheduling primitives. It is not a separate operator-selectable TaskChampion runtime path.

- **Sync database**
  A SQLite file used by the TaskChampion sync protocol. The served runtime chain is `users/<user-id>/merged/sync.sqlite`; old `users/<user-id>/sync.sqlite` files are legacy/non-serving artifacts.
...
- **Sync storage manager**
  The server component that caches open sync SQLite connections for TaskChampion sync handling. The gateway runtime uses the merged sync storage manager for `merged/sync.sqlite`.
...
Internally, the server keeps one canonical plaintext task database per user and one durable merged TaskChampion sync chain per user. Each registered device still has its own `client_id` and derived secret; the `/v1/client/*` adapter translates device envelopes at the HTTP edge and routes all TaskChampion traffic through `MergedSyncGateway`.
...
REST / iOS client
        |
        v
  Canonical replica (plaintext source truth)
        ^
        |
 MergedSyncGateway projection/apply
        |
        v
Merged TC sync chain (merged/sync.sqlite)
        ^
        |
...
- `merged/sync.sqlite`
  - the gateway-served TaskChampion sync DB used by Taskwarrior-compatible clients
- `merged/replica/taskchampion.sqlite3`
  - the durable merged projection cache used to build the protocol chain
- optionally `sync.sqlite` or `sync/<client-id>.sqlite`
...
## 3. Canonical Replica vs Merged Sync DB
...
The merged sync DB exists to support TaskChampion clients without making every client share one mutable credential or exposing canonical source truth directly.

Properties of the merged sync DB:

- one per user at `merged/sync.sqlite`
- served by the TaskChampion protocol endpoints through `MergedSyncGateway`
...
- one merged sync transport state on disk
...
### 3.2 Why The Runtime Uses One Gateway TaskChampion Sync DB
...
- one gateway-served per-user `merged/sync.sqlite`
...
### 3.3 The Real Role of the Merged Sync DB

It helps to think of the merged sync DB as the protocol-facing store for that user.

The canonical replica answers:

- what is the user’s current task state for REST and server-side operations?

The merged sync DB answers:
...
The serving translation between REST/canonical state and TaskChampion sync
traffic is done by the **merged sync gateway** (see §3.2–§3.3), not by a
separate bridge process. The term *sync bridge* now refers to the supporting
**scheduling and freshness** machinery that decides *when* the gateway projects
canonical changes into the shared sync DB and tracks which devices are already
caught up. It is internal machinery, not an operator-selectable runtime path.

What it coordinates:

- REST task create:
  - writes to the canonical replica
  - the scheduler queues a projection so the change reaches the shared sync DB
- TC device write:
  - is applied to the shared sync DB by the gateway
  - the gateway reconciles it back into canonical state

Without this coordination the REST world and the TaskChampion world would drift
apart. The next section covers the scheduler in detail.

...
The current sync model has two separate operator concerns behind the merged gateway:

- `admin sync` manages the user's canonical sync identity/secret material.
- `admin device` manages each gateway-bound physical device that is allowed to sync.
...
### Rotation recovery (lost device credential)

When a device credential is lost — for example a provisioning response never
reached the client, or a device must be re-keyed — use the atomic rotation path
rather than reasoning about partial state by hand:

```bash
cmdock-server admin device rotate <user-id> <client-id> --server-url https://tasks.example.com
```

`device rotate` revokes the old `client_id` and mints a replacement in one
operator action. The replacement is provisioned under the **operator admin
token's authority** (not the lost device's credential), and the server emits a
single doctor-visible, append-only audit entry (`action = "device.rotate"`,
`reason = "rotation_recovery"`, carrying `old_client_id`, the new `client_id`,
`user_id`, and `request_id`). After rotation the old credential can no longer
sync; the client re-onboards with the emitted snippet.

This is the supported lost-response recovery path for the merged gateway —
prefer it over a manual `revoke` + `create` pair so the recovery is captured as
a correlated audit event. See
[Admin Surfaces Reference](../reference/admin-surfaces-reference.md) and
[Audit Reference](../reference/audit-reference.md) for the endpoint and audit
field contract.
...
└── users/
    ...
    │   ├── merged/
    │   │   ├── replica/
    │   │   │   └── taskchampion.sqlite3 # Durable merged projection cache
    │   │   └── sync.sqlite              # Gateway-served TC sync chain
...
**Key properties:**
- Each user's canonical replica is separate from their merged-gateway TaskChampion sync DB
...
- Device delete affects device metadata; it does not remove the gateway `merged/sync.sqlite`
...
# Lost-credential recovery: atomic revoke-old + mint-replacement, one audit event
cmdock-server admin device rotate <user-id> <client-id> --server-url https://tasks.example.com

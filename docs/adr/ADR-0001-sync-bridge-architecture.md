...
> **Superseded in part:** ADR-0009 / server#143 replaces this ADR's released
> TW-sync runtime shape with `MergedSyncGateway` as the day-one sync
> architecture. The lower-level discoveries here remain relevant: TC's
> single-chain model, server-side key escrow, encryption envelope handling, and
> the need to keep REST/source truth separate from TW protocol storage.

...
use crate::store::models::{ContextRecord, ViewRecord};
...
/// Current version of the default contextset.
/// Bump this when adding new contexts or changing existing built-in
/// project_prefixes/label/color/icon. Reconciliation updates unmodified
/// builtins; user-modified contexts are preserved.
pub const CONTEXTSET_VERSION: i32 = 1;
...
/// Return the default built-in contexts for a new user.
///
/// These IDs match the `context_id` references on the project-scoped
/// default views (personal/work/health) so that auto-scoping works
/// out of the box. The `project_prefixes` deliberately match the
/// uppercase project names the v4 viewset hardcoded as filters
/// (`project:PERSONAL`, etc.) — users upgrading from v4 keep working
/// without re-tagging their tasks.
pub fn default_contexts() -> Vec<ContextRecord> {
    vec![
        ContextRecord {
            id: "personal".to_string(),
            label: "Personal".to_string(),
            project_prefixes: vec!["PERSONAL".to_string()],
            ...
            user_modified: false,
            hidden: false,
            template_version: CONTEXTSET_VERSION,
        },
        ContextRecord {
            id: "work".to_string(),
            label: "Work".to_string(),
            project_prefixes: vec!["WORK".to_string()],
        ...
        ContextRecord {
            id: "health".to_string(),
            label: "Health".to_string(),
            project_prefixes: vec!["HEALTH".to_string()],
...
pub fn builtin_context(id: &str) -> Option<ContextRecord> {
    default_contexts().into_iter().find(|c| c.id == id)
}

fn merge_builtin_context(existing: &ContextRecord, default: &ContextRecord) -> ContextRecord {
    if !existing.user_modified {
        return default.clone();
    }

    // User-modified — preserve user-editable fields, push server-owned
    // metadata (template_version, origin) forward.
    let mut merged = existing.clone();
    merged.origin = default.origin.clone();
    merged.template_version = CONTEXTSET_VERSION;
    merged
}

/// Reconcile built-in contexts for a user.
///
/// Called lazily on first `/api/contexts` and `/api/app-config` access.
/// Must run **before** `reconcile_default_views` so that the (future)
/// view validator sees seeded contexts.
///
/// Rules mirror `reconcile_default_views`:
/// - **Missing builtin**: create it (unless hidden tombstone exists)
/// - **Unmodified builtin with older version**: update to latest template
/// - **User-modified builtin**: preserve user-editable fields but still merge
///   server-owned metadata forward
/// - **Hidden (tombstoned) builtin**: leave alone (user explicitly deleted it)
/// - **User-created context with same ID as a builtin**: never overwrite
pub async fn reconcile_default_contexts(
    store: &dyn crate::store::ConfigStore,
    user_id: &str,
) -> anyhow::Result<()> {
    let existing = store.list_contexts_all(user_id).await?;
    let defaults = default_contexts();

    for default in &defaults {
        let existing_ctx = existing.iter().find(|c| c.id == default.id);

        match existing_ctx {
            None => {
                store.upsert_context(user_id, default).await?;
            }
            Some(c) if c.origin != "builtin" => {
                // User-created context with same ID — never overwrite.
                continue;
            }
            Some(c) if c.hidden => {
                // User deleted this builtin — respect the tombstone.
            ...
            Some(c) if c.template_version < CONTEXTSET_VERSION => {
                let merged = merge_builtin_context(c, default);
                store.upsert_context(user_id, &merged).await?;
            }
            Some(_) => {
                // Already at current version, not modified — nothing to do.
            }
...
mod tests {
    ...
    #[test]
    fn test_default_contexts_count() {
        assert_eq!(
            default_contexts().len(),
            3,
            "should have 3 default contexts"
        );
    }

    #[test]
    fn test_default_contexts_all_builtin() {
        for c in default_contexts() {
            assert_eq!(c.origin, "builtin");
            assert!(!c.user_modified);
            assert!(!c.hidden);
            assert_eq!(c.template_version, CONTEXTSET_VERSION);
        }
    }

    #[test]
    fn test_default_contexts_match_view_context_ids() {
        // Every project-scoped builtin view's context_id must resolve to a builtin context.
        let contexts = default_contexts();
        let context_ids: Vec<&str> = contexts.iter().map(|c| c.id.as_str()).collect();

        for v in default_views() {
            if let Some(cid) = &v.context_id {
                assert!(
                    context_ids.contains(&cid.as_str()),
                    "view {} references context_id={} but no matching default context exists",
                    v.id,
                    cid,
                );
            }
    ...
    #[test]
    fn test_default_contexts_have_uppercase_prefixes() {
        // Match v4 hardcoded filters (project:PERSONAL etc.) so users upgrading
        // don't lose visibility on already-tagged tasks.
        for (id, expected) in [
            ("personal", "PERSONAL"),
            ("work", "WORK"),
            ("health", "HEALTH"),
        ] {
            let c = default_contexts().into_iter().find(|c| c.id == id).unwrap();
            assert_eq!(
                c.project_prefixes,
                vec![expected.to_string()],
                "{id} should have prefix {expected}"
            );
        }
    }
}

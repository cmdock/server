...
pub async fn get_summary(
    ...
    let parsed_filter = filter::parse_filter_at(filter_str, eval_ctx.now);
    let mut matching_tasks: Vec<_> = all_tasks
        .values()
        .filter(|t| filter::matches_with_context(t, &parsed_filter, &eval_ctx))
        .map(|t| crate::tasks::projection::task_to_item(t, Some(&pending_uuids), None))
        .collect();
    // Sort by canonical-UUID String for prompt determinism. The LLM path
    // serialises `matching_tasks` straight into the prompt body; without this
    // sort, HashMap iteration order leaks into the prompt and defeats
    // Anthropic's prompt cache (cache key includes the prompt prefix bytes).
    // Mirrors the contract enforcement on `GET /api/tasks` (#102) — the
    // template-fallback path doesn't read the Vec, so this is a no-op there.
    matching_tasks.sort_unstable_by(|a, b| a.uuid.cmp(&b.uuid));

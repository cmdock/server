...
#[schema(example = json!({
    ...
    "notes": [
        "The merged sync DB is missing. It can be rebuilt from canonical state."
    ]
}))]
pub struct UserRecoveryAssessment {
    ...
    /// Wire field name is retained for admin-client compatibility; the value
    /// now describes the gateway-served `merged/sync.sqlite` DB.
...
pub async fn assess_user_recovery(
    ...
    let shared_sync_db_path = SyncStorage::merged_db_path(&user_dir);
    ...
    let status = if !missing_device_secrets.is_empty() {
        ...
        notes.push(format!(
            "The merged sync DB schema_version is newer than this binary supports ({actual_version} > {expected_sync_schema_version})."
        ));
        RecoveryStatus::NeedsOperatorAttention
    } else if (sync_identity_exists || !active_devices.is_empty()) && !shared_sync_db_exists {
        notes.push(
            "The merged sync DB is missing. It can be rebuilt from canonical state.".to_string(),
        );
        RecoveryStatus::Rebuildable
    } else if shared_sync_db_error.is_some() {
        notes.push(
            "The merged sync DB could not be inspected cleanly. It may need rebuild or operator review."
                .to_string(),
        );
        RecoveryStatus::Rebuildable
    } else if shared_sync_upgrade_needed {
        notes.push(
            "The merged sync DB is older than the current runtime storage level and needs uplift."
                .to_string(),
        );
        RecoveryStatus::Rebuildable
    } else {

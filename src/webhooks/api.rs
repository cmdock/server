...
// ADR-0002 HC-1 exception: 6 args, one over the threshold. Each is an
// independent input from the public webhook-registration request and
// they're plumbed straight to focused validators (validate_url,
// normalize_events, normalize_modified_fields, normalize_name,
// encrypt_secret). Bundling into a `RegistrationRequest` struct would
// add a layer that's not used elsewhere; the args are the wire shape.
...
pub fn map_user_store_error(
    ...
    if matches!(err, crate::store::StoreError::WebhookLimitReached { .. }) {
        return api_error(
            WebhookApiError::LimitReached,
            "Webhook limit reached for this user",
        );
    }
    if err.is_unique(crate::store::error::resources::WEBHOOKS_USER_URL) {
        return api_error(
...
pub fn map_admin_store_error(
    err: crate::store::StoreError,
) -> (StatusCode, Json<WebhookErrorResponse>) {
    if matches!(err, crate::store::StoreError::WebhookLimitReached { .. }) {
        return api_error(WebhookApiError::LimitReached, "Admin webhook limit reached");
    }
    if err.is_unique(crate::store::error::resources::ADMIN_WEBHOOKS_URL) {
        return api_error(
            WebhookApiError::DuplicateUrl,

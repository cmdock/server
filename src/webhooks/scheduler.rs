...
async fn poll_once_inner(state: &AppState, now: DateTime<Utc>) -> anyhow::Result<()> {
    ...
    for user in users {
        ...
        let (tasks, pending_uuids): (Vec<_>, std::collections::HashSet<uuid::Uuid>) = {
        ...
        for task in tasks {
            ...
            if is_due(due_at, now) {
                emit_time_event(state, &user.id, &task, "task.due", due_at, &pending_uuids).await?;
            }
...
async fn emit_time_event(
    ...
    event: &str,
    due_at: DateTime<Utc>,
    pending_uuids: &std::collections::HashSet<uuid::Uuid>,
) -> anyhow::Result<()> {
    let task_item = crate::tasks::projection::task_to_item(task, Some(pending_uuids), None);
    let inserted = state
        .store

...
// ADR-0002 HC-1 exception: 7 args, two over threshold. The event payload
// shape is naturally polymorphic across mutation kinds (some carry task,
// some carry sync summary, none carry both). Bundling into a single
// `EventPayload` enum-of-shapes is plausible but the current flat-args
// pattern keeps the caller side simple — each task mutation handler
// fills in only the fields that apply.
...
async fn deliver_event(
    ...
    let scope = resolve_event_envelope_scope(state, user_id, event).await;
...
async fn resolve_event_envelope_scope(
    state: &AppState,
    user_id: &str,
    event: &str,
) -> EventEnvelopeScope {
    if event == "webhook.test" {
        return EventEnvelopeScope::default();
    }

    let task_scope_id = match state.store.get_personal_task_scope_for_user(user_id).await {
        Ok(Some(scope)) => Some(scope.id),
        Ok(None) => {
            tracing::warn!(
                user_id = %user_id,
                event = %event,
                kind = "missing",
                "Webhook logical event has no Personal Task Scope; omitting task_scope_id"
            );
            None
        }
        Err(err) => {
            tracing::warn!(
                ...
                kind = "error",
                error = %err,
                "Failed to resolve Personal Task Scope for webhook logical event; omitting task_scope_id"
            );
            None
        }
    };

    EventEnvelopeScope { task_scope_id }
}
...
async fn deliver(
    ...
    for attempt in 1..=max_attempts {
        ...
        let payload = DeliveryPayload {
            ...
            task_scope_id: scope.task_scope_id.clone(),

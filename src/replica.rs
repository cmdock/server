...
use taskchampion::{Operations, Replica, SqliteStorage, Tag};
...
pub fn apply_parsed_fields(
    ...
    if let Some(ref due_str) = parsed.due {
        ...
        if let Some(dt) = crate::tasks::dates::parse_date_value(due_str) {
            task.set_due(Some(dt), ops)?;
        }
    }
    if let Some(ref wait_str) = parsed.wait {
        // Same broad parser as `due:` — the contract's `parse_raw` § applies
        // here (named dates, ISO, relative durations, canonical TW format).
        if let Some(dt) = crate::tasks::dates::parse_date_value(wait_str) {
            task.set_wait(Some(dt), ops)?;
        }
    }
    if let Some(ref scheduled_str) = parsed.scheduled {
        // TC stores `scheduled` as a generic property holding epoch seconds
        // (see `parse_task_scheduled` in tasks/mod.rs). Convert from
        // DateTime → epoch seconds string before set_value.
        if let Some(dt) = crate::tasks::dates::parse_date_value(scheduled_str) {
            task.set_value("scheduled", Some(dt.timestamp().to_string()), ops)?;
        }
    }
    Ok(())
}

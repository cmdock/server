...
pub enum ProvisionDeviceError {
    ...
    MissingStoredSecret,
    /// `bootstrap_request_id` already used by an existing device. Caller
    /// validates whether the existing device matches the new request and
    /// either re-issues credentials or surfaces a 409 conflict.
    #[error("bootstrap request id already used")]
    BootstrapRequestConflict,
    #[error(transparent)]
    Internal(#[from] anyhow::Error),
}

impl From<crate::store::StoreError> for ProvisionDeviceError {
    fn from(err: crate::store::StoreError) -> Self {
        use crate::store::error::resources;
        if err.is_unique(resources::DEVICES_BOOTSTRAP_REQUEST_ID) {
            return ProvisionDeviceError::BootstrapRequestConflict;
        }
        ProvisionDeviceError::Internal(anyhow::Error::from(err))
    }
}
...
/// Rotate a gateway-bound TW identity by minting a replacement device
/// credential and revoking the old client id. The TaskChampion client id is
/// part of the PBKDF salt, so rotation must produce a new `client_id`; changing
/// only the stored encrypted secret for the same id would be ineffective for
/// existing clients.
pub async fn rotate_owned_device(
    store: &dyn ConfigStore,
    data_dir: &Path,
    user_id: &str,
    client_id: &str,
    master_key: Option<[u8; 32]>,
) -> Result<ProvisionedDevice, DeviceLifecycleError> {
    let old = load_owned_device(store, user_id, client_id).await?;
    let rotated_name = format!("{} (rotated)", old.name);
    let replacement = provision_device(store, data_dir, user_id, &rotated_name, master_key).await?;
    set_owned_device_revoked(store, user_id, &old.client_id, true).await?;
    Ok(replacement)
}

// ADR-0002 HC-1 exception: 6 args, one over the threshold. Args split
// into three independent concerns — store handle, identity (user_id +
// name + data_dir), and crypto/bootstrap metadata. `bootstrap: Option<...>`
// already groups the bootstrap-specific fields (request_id, username,
// expires_at, create_user_if_missing); the remaining args are the
// minimum surface for a non-bootstrap device.
...
pub async fn provision_device_with_metadata(
    ...
    crate::merged_sync_gateway::storage::ensure_merged_sync_storage(data_dir, user_id)?;

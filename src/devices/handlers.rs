...
pub async fn register_device(
    ...
    .map_err(|e| match e {
        ...
        crate::devices::service::ProvisionDeviceError::BootstrapRequestConflict => {
            // Direct device-register path doesn't supply a bootstrap_request_id,
            // so this should never fire here. Treat as internal if it does.
            tracing::error!("unexpected bootstrap conflict on direct device register");
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                "Internal error".to_string(),
            )
...
pub async fn revoke_device(
    ...
    .await
    {
        Ok(()) => {
            state
                .recovery_runtime
                .evict_device(&auth.user_id, &client_id);

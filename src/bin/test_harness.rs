...
const SEED_TASKS: &[&str] = &[
    ...
    // Single dep: Deploy blocked by Review PR
    ...
    // Multi-step chain: Design → Implement → Write tests
    "project:FEATURE priority:M Design notification API schema",
    "project:FEATURE priority:M due:eow Implement notification endpoints",
    "project:FEATURE priority:L Write notification integration tests",
    // Multiple deps: Sprint planning blocked by BOTH shared drive AND dentist
...
// UDA seeding requires TW CLI (the REST raw parser doesn't parse UDAs —
// matches TW behaviour where UDAs need .taskrc declarations). After running
// `just seed`, apply UDAs with:
//   ./scripts/seed-udas.sh <ssh-host> <server-url> <user-id> <client-id> <secret>

/// Dependency chains created after seeding.
/// Each entry is (blocker_description_substring, blocked_description_substring).
/// The seed function looks up UUIDs by matching description text among
/// +tc_harness_seed tagged tasks only.
///
/// Chain variety:
///   - Single dep:    Deploy staging fixes → Review PR
///   - Multi-step:    Design → Implement → Write tests
///   - Multiple deps: Sprint planning → {Organise shared drive, Book dentist}
const SEED_DEPENDENCIES: &[(&str, &str)] = &[
    // Single dep
    ("Review PR for auth module", "Deploy staging fixes"),
    // Multi-step chain: Design → Implement → Write tests
    (
        "Design notification API schema",
    ...
    // Multiple deps on one task
    ("Organise shared drive", "Sprint planning"),
    ("Book dentist appointment", "Sprint planning"),
];
...
async fn cmd_seed(url: &str, token: &str) -> anyhow::Result<()> {
    ...
    // Group by blocked task so tasks with multiple deps get a single modify
    // call with all blockers (modify replaces the dep list, not appends).
    {
        println!();
        println!("{}", "=== Wiring Dependencies ===".bold());

        let tasks: Vec<TaskItem> = get(&client, url, token, "/api/tasks").await?;
        let seed_tasks: Vec<&TaskItem> = tasks
            .iter()
            .filter(|t| t.tags.iter().any(|tag| tag == SEED_TAG))
            .collect();
        let find_uuid = |desc_substr: &str| -> Option<String> {
            seed_tasks
                .iter()
                .find(|t| t.description.contains(desc_substr))
                .map(|t| t.uuid.clone())
        };

        // Group: blocked_desc → vec of blocker UUIDs
        let mut grouped: std::collections::BTreeMap<&str, Vec<String>> =
            std::collections::BTreeMap::new();
        let mut skipped = 0;
        for (blocker_desc, blocked_desc) in SEED_DEPENDENCIES {
            let Some(blocker_uuid) = find_uuid(blocker_desc) else {
                println!("  {} blocker not found: {blocker_desc:?}", "SKIP".yellow());
                skipped += 1;
                continue;
            };
            if find_uuid(blocked_desc).is_none() {
                println!("  {} blocked not found: {blocked_desc:?}", "SKIP".yellow());
                skipped += 1;
                continue;
            }
            grouped.entry(blocked_desc).or_default().push(blocker_uuid);
        }

        let mut dep_ok = 0;
        let mut dep_fail = 0;
        for (blocked_desc, blocker_uuids) in &grouped {
            let blocked_uuid = find_uuid(blocked_desc).unwrap();
            let body = serde_json::json!({"depends": blocker_uuids});
            match post_json::<TaskActionResponse>(
                &client,
                url,
                token,
                &format!("/api/tasks/{blocked_uuid}/modify"),
                &body,
            )
            .await
            {
                Ok(resp) if resp.success => {
                    println!(
                        "  {} {blocked_desc} depends on {} task(s)",
                        "linked".green(),
                        blocker_uuids.len()
                    );
                    dep_ok += blocker_uuids.len();
                }
                Ok(resp) => {
                    println!("  {} {blocked_desc}: {}", "FAIL".red(), resp.output);
                    dep_fail += blocker_uuids.len();
                }
                Err(e) => {
                    println!("  {} {blocked_desc}: {e}", "FAIL".red());
                    dep_fail += blocker_uuids.len();
                }
            }
        }
        println!("Linked: {dep_ok}  Failed: {dep_fail}  Skipped: {skipped}");
    }

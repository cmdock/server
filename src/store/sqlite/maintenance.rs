...
mod tests {
    ...
    /// SqliteConfigStore implements OperatorMaintenanceBackend; checkpoint,
    /// backup, restore round-trip via the trait dispatch path.
    #[tokio::test]
    async fn maintenance_trait_round_trip() {
        let tmp = TempDir::new().unwrap();
        let src_db = tmp.path().join("config.sqlite");
        let store = Arc::new(
            SqliteConfigStore::new(&src_db.to_string_lossy())
                .await
                .unwrap(),
        );
        store.run_migrations().await.unwrap();

        let maintenance: Arc<dyn OperatorMaintenanceBackend> = store.clone();

        // Checkpoint should succeed on a freshly-migrated DB.
        maintenance.checkpoint().await.unwrap();

        // Backup writes a usable SQLite file.
        let backup_path = tmp.path().join("backup.sqlite");
        maintenance.backup_to_path(&backup_path).await.unwrap();
        assert!(backup_path.exists());
        assert!(backup_path.metadata().unwrap().len() > 0);

        // Restore reads the backup back into the live DB without panicking.
        maintenance.restore_from_path(&backup_path).await.unwrap();
    }
}

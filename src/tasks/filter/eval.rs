...
fn eval_virtual_tag(task: &Task, tag_name: &str, ctx: &EvalContext) -> bool {
    ...
    if tag_name.eq_ignore_ascii_case("READY") {
        return task.get_status() == Status::Pending
            ...
            && crate::tasks::parse_task_scheduled(task)
                .map(|dt| dt <= now)
                .unwrap_or(true);
    }
...
fn eval_attribute(
    ...
    match name {
        ...
        "due" => eval_date_attr(task.get_due(), modifier, parsed_date),
        "entry" => eval_date_attr(task.get_entry(), modifier, parsed_date),
        "modified" => eval_date_attr(task.get_modified(), modifier, parsed_date),
        "wait" => eval_date_attr(task.get_wait(), modifier, parsed_date),
        "scheduled" => eval_date_attr(
            crate::tasks::parse_task_scheduled(task),
            modifier,
            parsed_date,
        ),
...
fn eval_date_attr(
    ...
    // C2 (#127) populates `parsed_date` for every DATE_ATTRIBUTES token via
    // `parse_filter_at`. A `None` here means the supplied filter value didn't
    // parse (e.g. `due:notadate`), which can never match any task — return
    // false silently.
    let filter_dt = match pre_parsed {
        Some(dt) => dt,
        None => return false,
    };
...
mod tests {
    ...
    /// Pin C3 (#127) eval contract: `eval_date_attr` trusts the AST's
    /// `parsed_date` and does **not** reparse `filter_val` at eval time.
    ///
    /// Load-bearing assertion: pass an *absolute* parseable filter value
    /// that resolves exactly to the task's due date, with `parsed_date:
    /// None`. Under the pre-C3 reparse path, `parse_date_value("20260329…")`
    /// would resolve to the same instant as the task's due, the `Equals`
    /// modifier would match (same calendar day), and the assertion would
    /// flip. After C3 removed the reparse, the evaluator returns false
    /// silently. Using TW's canonical `YYYYMMDDTHHmmssZ` format makes the
    /// resolved instant deterministic — independent of any wall-clock read
    /// the old fallback might have done — so the test can't false-pass
    /// just because `Utc::now()` happens to disagree with `fixed_now()`.
    ///
    /// Defence-in-depth: also pin the legitimate `parsed_date: None`
    /// production path (unparseable filter value, e.g. `due:notadate`) for
    /// both `Equals` and `Before` modifiers. No panic, no fallback.
    #[tokio::test]
    async fn eval_date_attr_does_not_reparse_filter_val_at_eval_time() {
        let storage = InMemoryStorage::new();
        let mut replica = Replica::new(storage);
        let now = fixed_now();
        let task_due = now.date_naive().and_hms_opt(0, 0, 0).unwrap().and_utc();
        let uuid = make_task_with_due(&mut replica, "task at fixed_now", Some(task_due)).await;
        let task = replica.get_task(uuid).await.unwrap().unwrap();
        let ctx = ctx_at(now);

        // (1) Load-bearing: absolute parseable value that resolves to the
        // task's due instant. Pre-C3 the reparse would match; post-C3 it
        // doesn't. `fixed_now()` is 2026-03-29 12:00 UTC; the task due is
        // 2026-03-29 00:00 UTC; the filter is 2026-03-29 00:00 UTC in TW
        // canonical form — same calendar day → would match under Equals
        // if any reparse fired.
        let canonical_filter_val = "20260329T000000Z";
        assert!(
            !eval_attribute(
                &task,
                "due",
                &AttrModifier::Equals,
                canonical_filter_val,
                None,
                &ctx,
            ),
            "eval_date_attr must not reparse filter_val at eval time — \
             absolute parseable value matching the task date with \
             parsed_date=None must yield false (would match under the \
             pre-C3 reparse path)"
        );

        // (2) Unparseable values — defence-in-depth.
        assert!(
            !eval_attribute(&task, "due", &AttrModifier::Equals, "notadate", None, &ctx),
            "unparseable date value must never match"
        );
        assert!(
            !eval_attribute(&task, "due", &AttrModifier::Before, "notadate", None, &ctx),
            "unparseable date value with Before modifier must never match"
        );
    }

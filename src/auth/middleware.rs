...
use crate::admin::services::connect_config::{ConnectConfigService, UseContext, UseOutcome};
...
///
/// Connect-config correlation is intentionally NOT cached here: the only
/// caller that needs it is the rejection-telemetry path on `auth.failure`
/// and `enforce_runtime_access` rejection, which is rare enough that an
/// extra service lookup is fine. Keeping the cache shape generic preserves
/// the §Independence boundary — connect-config concerns stay in the
/// service, not bleeding into the bearer-auth fast path. (#123)
#[derive(Clone)]
...
impl AuthCache {
    ...
    async fn put(&self, token: String, user_id: String, username: String, created_at: String) {
        let mut cache = self.cache.lock().await;
        cache.put(
            token,
            CachedIdentity {
...
fn log_connect_config_token_rejected(
    ...
    state: &AppState,
    path: &str,
    correlation: &LabeledTokenCorrelation,
    reason: &str,
) {
    tracing::error!(
        target: "boundary",
...
impl FromRequestParts<AppState> for AuthUser {
    ...
    async fn from_request_parts(
        ...
        // Service handle for connect-config correlation lookups.
        // Cheap to construct: holds an Arc clone of the store.
        let connect_config_svc = ConnectConfigService::new(state.store.clone());
        ...
        if let Some(entry) = state.auth_cache.get(token).await {
            ...
            .await
            {
                // Rejection on the cache-hit hot path — look up the
                // connect-config correlation now (uncached) so we can
                // attach it to the rejection telemetry. The lookup hits
                // the store, but only on rejection, which is rare.
                if let Ok(Some(correlation)) = connect_config_svc.lookup_correlation(token).await {
        ...
        let connect_config = connect_config_svc
        ...
        let client_ip =
            audit::client_ip(&parts.headers, state.config.server.trust_forwarded_headers);
        let use_ctx = UseContext {
            client_ip: client_ip.clone(),
            request_id: audit::request_id(&parts.headers),
            request_path: request_path.clone(),
        };
        match connect_config_svc.record_use(token, &use_ctx).await {
            Ok(UseOutcome::FirstUse(identity)) => {
                let correlation = LabeledTokenCorrelation {
            ...
            Ok(UseOutcome::RepeatUse(_)) | Ok(UseOutcome::NotConnectConfig) => {}
            Err(err) => {
                tracing::warn!("Failed to record connect-config token use: {err}");
            }
        }

        // Cache only generic runtime identity fields (no connect-config
        // semantics — see CachedIdentity doc comment).
        state
            .auth_cache
            .put(token.to_string(), user.id, user.username, user.created_at)
            .await;

...
pub async fn create_canonical_sync_identity(
    ...
    match store
        ...
        .await
    {
        Ok(()) => Ok(CreatedCanonicalSyncIdentity { client_id }),
        Err(err) if err.is_unique(crate::store::error::resources::REPLICAS_USER_ID) => {
            // Race: another writer slipped a replica in between the
            // pre-flight check and our insert. Surface as AlreadyExists
            // so callers fall through to the "load existing" path.
            let existing = store.get_replica_by_user(user_id).await?.ok_or_else(|| {
                SyncIdentityError::Internal(anyhow::anyhow!(
                    "replica unique constraint hit but row not found on reload"
                ))
            })?;
            Err(SyncIdentityError::AlreadyExists(existing.id))
        }
        Err(err) => Err(SyncIdentityError::Internal(anyhow::Error::from(err))),
    }
}

...
/// User-owned tables that single-user restore should clean and repopulate.
/// This list is parent-before-child for inserts; deletes iterate it in reverse.
/// Selective restore remains Personal Task Scope-only: future Team Task Scope
/// rows must not be copied by this user-scoped operator flow.
// Trusted SQL fragment used only with the bound `?1` Runtime User id parameter;
// every caller interpolating this constant must bind user_id to `?1`. Keeps the
// selective-restore invariant greppable: Team Task Scope rows are not copied.
const PERSONAL_TASK_SCOPE_FILTER: &str = "kind = 'personal' AND owner_runtime_user_id = ?1";

const USER_OWNED_TABLES_PARENT_FIRST: &[&str] = &[
    "api_tokens",
    "views",
    "contexts",
    "presets",
...
fn delete_user_rows_from_table(
    ...
    if table == "task_scopes" {
        delete_personal_task_scope_rows(tx, user_id)?;
    } else {
        tx.execute(
...
fn delete_personal_task_scope_rows(
    tx: &rusqlite::Transaction<'_>,
    user_id: &str,
) -> anyhow::Result<()> {
    tx.execute(
        &format!("DELETE FROM task_scopes WHERE {PERSONAL_TASK_SCOPE_FILTER}"),
        [user_id],
    )?;
    Ok(())
}

fn insert_task_key_allocations_from_backup(
    tx: &rusqlite::Transaction<'_>,
    user_id: &str,
) -> anyhow::Result<usize> {
    let backup_has_task_scope_ids =
        sqlite_column_exists(tx, "backup", "task_key_allocations", "task_scope_id")
            && sqlite_table_exists(tx, "backup", "task_scopes")?;
    let sql = if backup_has_task_scope_ids {
        "WITH source AS (
             SELECT * FROM backup.task_key_allocations WHERE user_id = ?1
         ),
         allowed_personal_scope AS (
             SELECT id FROM backup.task_scopes
             WHERE kind = 'personal' AND owner_runtime_user_id = ?1
         )
         INSERT INTO task_key_allocations
             (user_id, prefix, n, task_uuid, state, attempt_id,
              created_at, committed_at, task_scope_id)
         SELECT source.user_id, source.prefix, source.n, source.task_uuid,
                source.state, source.attempt_id, source.created_at,
                source.committed_at,
                CASE
                    WHEN allowed_personal_scope.id IS NOT NULL
                    THEN source.task_scope_id
                    ELSE NULL
                END
         FROM source
         LEFT JOIN allowed_personal_scope
           ON allowed_personal_scope.id = source.task_scope_id"
            .to_string()
    } else {
        "INSERT INTO task_key_allocations
             (user_id, prefix, n, task_uuid, state, attempt_id,
              created_at, committed_at, task_scope_id)
         SELECT user_id, prefix, n, task_uuid, state, attempt_id,
                created_at, committed_at, NULL
         FROM backup.task_key_allocations WHERE user_id = ?1"
            .to_string()
    };
    let inserted = tx.execute(&sql, [user_id])?;

    if backup_has_task_scope_ids {
        let nulled: i64 = tx.query_row(
            "SELECT COUNT(*)
             FROM backup.task_key_allocations allocation
             WHERE allocation.user_id = ?1
               AND allocation.task_scope_id IS NOT NULL
               AND NOT EXISTS (
                   SELECT 1 FROM backup.task_scopes scope
                   WHERE scope.id = allocation.task_scope_id
                     AND scope.kind = 'personal'
                     AND scope.owner_runtime_user_id = allocation.user_id
               )",
            [user_id],
            |row| row.get(0),
        )?;
        if nulled > 0 {
            tracing::info!(
                user_id,
                nulled,
                "Selective restore nulled task_key_allocations.task_scope_id values for scopes not restored by the user-scoped flow"
            );
        }
    }
        ...
        "task_scopes" => tx.execute(
            &format!(
                "INSERT INTO task_scopes
                 SELECT * FROM backup.task_scopes
                 WHERE {PERSONAL_TASK_SCOPE_FILTER}"
            ),
            [user_id],
        )?,
        "task_key_allocations" => insert_task_key_allocations_from_backup(tx, user_id)?,
...
fn restore_single_user_config(
    ...
    for &table in USER_OWNED_TABLES_PARENT_FIRST.iter().rev() {
    ...
    for table in USER_OWNED_TABLES_PARENT_FIRST {
...
pub(super) async fn run_restore(
    ...
    if let Some(user_id) = user_id {
        // Local selective restore is an offline/break-glass operator path that
        // trusts the operator-selected backup directory. Unlike service-level
        // restore, legacy CLI backups have no manifest to validate, so Task
        // Scope compatibility is enforced by table-scoped copy rules below.

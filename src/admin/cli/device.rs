...
use crate::devices::service::{
    delete_owned_device, rename_owned_device, rotate_owned_device, set_owned_device_revoked,
};
...
pub(super) async fn run(action: DeviceAction, data_dir: &Path) -> anyhow::Result<()> {
    ...
    match action {
        ...
        DeviceAction::Show { user_id, client_id } => {
            ...
            let source_account = store
                .get_user_prefix(&user_id)
                .await?
                .unwrap_or_else(|| "personal".to_string());
            println!("Gateway-bound TW identity for user {user_id}:");
            println!("  Runtime:       mergedGateway");
        ...
        DeviceAction::Rotate {
            ...
            if !yes
                && !confirm(&format!(
                    "Rotate device '{}' ({})? The old client_id will be revoked. [y/N] ",
                    device.name, device.client_id
                ))?
            {
                println!("Cancelled.");
            ...
            let provisioned = rotate_owned_device(
                &*store,
                data_dir,
                &user_id,
                &device.client_id,
            ...
            println!("Device credential rotated:");
            println!("  User:               {user_id}");
            println!("  Old Client ID:      {} (revoked)", device.client_id);
            println!("  New Client ID:      {}", provisioned.client_id);
            println!("  New Name:           {}", provisioned.name);
            println!(
                "  Encryption Secret:  {}",
                provisioned.encryption_secret_hex
            );

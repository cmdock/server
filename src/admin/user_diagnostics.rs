...
use crate::admin::users::{
    IntegrityMode, IntegrityResult, MergedSyncDiagnostics, MergedSyncJournalResponse,
    MergedSyncJournalStateCountResponse, UserStats, UserStatsQuery,
};
...
pub async fn user_stats(
    ...
    let merged_sync = collect_merged_sync_diagnostics(&state, &user_id)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn collect_merged_sync_diagnostics(
    state: &AppState,
    user_id: &str,
) -> anyhow::Result<MergedSyncDiagnostics> {
    let counts = state
        .store
        .count_merged_sync_journal_states_for_user(user_id)
        .await?
        .into_iter()
        .map(|row| MergedSyncJournalStateCountResponse {
    ...
    let recent_journal = state
        .store
        .list_merged_sync_journal_for_user(user_id, 20)
        .await?
        .into_iter()
        .map(|row| MergedSyncJournalResponse {
            ...
            state: row.state.as_str().to_string(),
            recovery_status: row.recovery_status.as_str().to_string(),
    ...
    Ok(MergedSyncDiagnostics {
        journal_state_counts: counts,
        recent_journal,
    })
}
...
async fn collect_integrity(mode: IntegrityMode, replica_dir: &std::path::Path) -> IntegrityResult {
    ...
    if shared_sync_db.exists() {
        if let Some(result) = run_integrity_for_path(shared_sync_db, pragma_str.clone()).await {
            sync_results.push(format!("sync.sqlite: {result}"));
        }
    }
    let merged_sync_db = replica_dir.join("merged").join("sync.sqlite");
    if merged_sync_db.exists() {
        if let Some(result) = run_integrity_for_path(merged_sync_db, pragma_str).await {
            sync_results.push(format!("merged/sync.sqlite: {result}"));
        }
    }

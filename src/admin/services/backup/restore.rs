...
fn stage_user_files(
    ...
    for entry in &manifest.contents.replicas {
        let replica_entry = entry
            .replica_db
            .as_ref()
            .or(entry.canonical_task_db.as_ref());
        if let Some(replica_db) = replica_entry {
        ...
        if let Some(sync_db) = sync_entry {
            ...
            let sync_path = Path::new(&sync_db.file);
            let dst = if sync_path
                .components()
                .any(|component| component.as_os_str() == "merged")
            {
                staging_root.join(&sync_db.file)
            } else {
                staging_root
                    .join("users")
                    .join(&entry.user_id)

...
use crate::admin::prefix::{apply_prefix, derive_prefix, SOURCE_BOOTSTRAP};
...
impl BootstrapService {
    ...
    pub async fn bootstrap_user_device(
        ...
        .await
        {
            ...
            Err(ProvisionDeviceError::BootstrapRequestConflict) => {
                let existing = self
                    .store
                    .get_device_by_bootstrap_request(&bootstrap_request_id)
                    .await?
                    .ok_or_else(|| {
                        anyhow::anyhow!("bootstrap_request_id race detected but device not found")
                    })?;
                self.validate_existing_request(
                    &existing,
                    &user,
            ...
            (None, Some(username)) => {
                    ...
                    .await
                {
                    Ok(user) => {
                        ...
                        .await?;
                        // Mirror `src/admin/cli/user.rs::run` (in-container CLI):
                        // every user MUST have a `prefix` assigned at creation
                        // time so Phase 4 task-key migration (`#130`) doesn't
                        // 500 on first read. Bootstrap was missed in Phase 1
                        // and only surfaced when Phase 4 made the missing-
                        // prefix case fatal — see `cmdock/server#137`.
                        assign_derived_prefix(self.store.as_ref(), &user)
                            .await
                            .map_err(BootstrapError::Internal)?;
                        Ok((user, true))
                    }
                    Err(err) if err.is_unique(crate::store::error::resources::USERS_USERNAME) => {
                        let user = self
                            .store
                    ...
                    Err(err) => Err(BootstrapError::Internal(err.into())),
                }
            }
...
/// Snapshot taken prefixes, derive a fresh one from the username + UUID,
/// and apply it. Mirrors `src/admin/cli/user.rs::run`'s no-`--prefix`
/// branch — the only difference is `source = "bootstrap"` for the audit
/// trail, distinguishing HTTP-bootstrap user creation from in-container
/// CLI signup.
async fn assign_derived_prefix(store: &dyn ConfigStore, user: &UserRecord) -> anyhow::Result<()> {
    ...
    apply_prefix(store, &user.id, &derived, SOURCE_BOOTSTRAP).await?;
    Ok(())
}

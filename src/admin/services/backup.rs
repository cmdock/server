...
pub(super) const CURRENT_CONFIG_SCHEMA_VERSION: i64 = 33;
...
pub(super) struct ReplicaBackupEntry {
    user_id: String,
    username: String,
    /// Personal Task Scope metadata for Task Scope-aware restore checks. Older
    /// manifests omit this block; restore continues to accept them.
    #[serde(skip_serializing_if = "Option::is_none")]
    task_scope: Option<TaskScopeBackupEntry>,
    /// Compatibility name for the canonical per-user TaskChampion DB.
    #[serde(skip_serializing_if = "Option::is_none")]
    replica_db: Option<BackupFileEntry>,
    /// Task Scope vocabulary name for the same canonical TaskChampion DB.
    #[serde(skip_serializing_if = "Option::is_none")]
    canonical_task_db: Option<BackupFileEntry>,
    /// Compatibility name for the per-runtime-user merged sync DB.
    #[serde(skip_serializing_if = "Option::is_none")]
    sync_db: Option<BackupFileEntry>,
    /// Task Scope vocabulary name for the same per-runtime-user merged sync DB.
    #[serde(skip_serializing_if = "Option::is_none")]
    merged_sync_db: Option<BackupFileEntry>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(Debug, Serialize, Deserialize, Clone)]
pub(super) struct TaskScopeBackupEntry {
...
pub(super) struct BackupSecrets {
    ...
    /// True when the `config.sqlite` file in this backup contains the encrypted
    /// webhook signing-secret rows (`webhooks.secret_enc`,
    /// `admin_webhooks.secret_enc`). Webhook signing capability is restored when
    /// the server is started with the same `master_key` that was in use when the
    /// webhooks were registered. Plaintext secrets are intentionally not included
    /// in backups per the architecture prohibition.
    #[serde(default)]
    webhook_signing_via_encrypted_config_db: bool,
}
...
#[cfg(test)]
mod tests {
    use super::CURRENT_CONFIG_SCHEMA_VERSION;

    #[test]
    fn config_schema_version_matches_highest_migration_file() {
        let max_migration = std::fs::read_dir("migrations")
            .expect("migrations directory must exist for schema-version test")
            ...
            .max()
            .expect("at least one migration file must exist");
        assert_eq!(CURRENT_CONFIG_SCHEMA_VERSION, max_migration);
    }
}

    ...
    pub async fn ensure_record_for_user(
        ...
        let created = match self.create_for_user(user_id, master_key).await {
            ...
            Err(sync_identity::SyncIdentityError::AlreadyExists(_)) => {
                // Covers both pre-flight detection and the post-insert
                // race surfaced via `replicas.user_id` UNIQUE violation
                // (see `sync_identity::create_canonical_sync_identity`).

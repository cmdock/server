...
pub struct ServerConfig {
    ...
    #[serde(default)]
    pub task_read: TaskReadSection,
    #[serde(default)]
    pub task_write: TaskWriteSection,
...
/// `[task_read]` config section — controls read-endpoint limits per
/// `task-read-contract.md` § Configuration Model.
#[derive(Debug, Deserialize, Clone)]
pub struct TaskReadSection {
    /// Hard cap on raw entries in `GET /api/tasks?uuids=<csv>`. Applied
    /// before deduplication. Default 100 per the contract.
    #[serde(default = "default_batch_max_uuids")]
    pub batch_max_uuids: usize,
}

impl Default for TaskReadSection {
    fn default() -> Self {
        Self {
            batch_max_uuids: default_batch_max_uuids(),
        }
    }
}

fn default_batch_max_uuids() -> usize {
    100
}

/// `[task_write]` config section — controls Idempotency-Key behaviour per
/// `task-write-contract.md` § Idempotency § Retention (commit a3f242a).
#[derive(Debug, Deserialize, Clone)]
pub struct TaskWriteSection {
    /// Retention window for `completed` dedup records. After this, retries
    /// with the same key are treated as fresh requests. Default 24h covers
    /// realistic offline-overnight mobile retry horizons.
    #[serde(default = "default_idempotency_retention_hours")]
    pub idempotency_retention_hours: u32,
    /// Stranded-`pending` timeout. Phase 3 may fail (process crash,
    /// config-DB error after Phase 2 commit), leaving a `pending` row that
    /// would otherwise block all retries indefinitely. Lookup-time expiry
    /// treats rows past this age as expired regardless of reaper state.
    /// Default 300s (5min) — must be comfortably above max expected
    /// Phase 2 mutation duration. Slow Phase 2 exceeding this timeout has
    /// the same residual-window failure mode as crashed Phase 3.
    #[serde(default = "default_idempotency_pending_timeout_seconds")]
    pub idempotency_pending_timeout_seconds: u32,
    /// `Retry-After` header value (seconds) on `503 IDEMPOTENCY_IN_FLIGHT`
    /// responses. Default 5s.
    #[serde(default = "default_idempotency_retry_after_seconds")]
    pub idempotency_retry_after_seconds: u32,
}

impl Default for TaskWriteSection {
    fn default() -> Self {
        Self {
            idempotency_retention_hours: default_idempotency_retention_hours(),
            idempotency_pending_timeout_seconds: default_idempotency_pending_timeout_seconds(),
            idempotency_retry_after_seconds: default_idempotency_retry_after_seconds(),
        }
    }
}

fn default_idempotency_retention_hours() -> u32 {
    24
}
fn default_idempotency_pending_timeout_seconds() -> u32 {
    300
}
fn default_idempotency_retry_after_seconds() -> u32 {
    5
}
...
impl ServerConfig {
    ...
    ///   CMDOCK_TASK_READ_BATCH_MAX_UUIDS → task_read.batch_max_uuids (positive integer; ignored otherwise)
    ///   CMDOCK_TASK_WRITE_IDEMPOTENCY_RETENTION_HOURS → task_write.idempotency_retention_hours (positive integer; ignored otherwise)
    ///   CMDOCK_TASK_WRITE_IDEMPOTENCY_PENDING_TIMEOUT_SECONDS → task_write.idempotency_pending_timeout_seconds (positive integer; ignored otherwise)
    ///   CMDOCK_TASK_WRITE_IDEMPOTENCY_RETRY_AFTER_SECONDS → task_write.idempotency_retry_after_seconds (positive integer; ignored otherwise)
    pub fn apply_env_overrides(&mut self) {
        ...
        if let Ok(v) = std::env::var("CMDOCK_TASK_READ_BATCH_MAX_UUIDS") {
            if let Ok(n) = v.parse::<usize>() {
                if n > 0 {
                    self.task_read.batch_max_uuids = n;
                }
            }
        }
        if let Ok(v) = std::env::var("CMDOCK_TASK_WRITE_IDEMPOTENCY_RETENTION_HOURS") {
            if let Ok(n) = v.parse::<u32>() {
                if n > 0 {
                    self.task_write.idempotency_retention_hours = n;
                }
            }
        }
        if let Ok(v) = std::env::var("CMDOCK_TASK_WRITE_IDEMPOTENCY_PENDING_TIMEOUT_SECONDS") {
            if let Ok(n) = v.parse::<u32>() {
                if n > 0 {
                    self.task_write.idempotency_pending_timeout_seconds = n;
                }
            }
        }
        if let Ok(v) = std::env::var("CMDOCK_TASK_WRITE_IDEMPOTENCY_RETRY_AFTER_SECONDS") {
            if let Ok(n) = v.parse::<u32>() {
                if n > 0 {
                    self.task_write.idempotency_retry_after_seconds = n;
                }
            }
        }
...
mod tests {
    ...
    fn default_config() -> ServerConfig {
        ServerConfig {
            ...
            task_read: TaskReadSection::default(),
            task_write: TaskWriteSection::default(),
            master_key: None,
        }
    }
...
"#,
    ...
    fn test_env_overrides() {
        ...
        // --- task_read.batch_max_uuids env override (positive integer) ---
        let mut cfg = default_config();
        unsafe { std::env::set_var("CMDOCK_TASK_READ_BATCH_MAX_UUIDS", "250") };
        cfg.apply_env_overrides();
        unsafe { std::env::remove_var("CMDOCK_TASK_READ_BATCH_MAX_UUIDS") };
        assert_eq!(cfg.task_read.batch_max_uuids, 250);

        // --- task_read.batch_max_uuids env override (zero — must be ignored) ---
        let mut cfg = default_config();
        let original = cfg.task_read.batch_max_uuids;
        unsafe { std::env::set_var("CMDOCK_TASK_READ_BATCH_MAX_UUIDS", "0") };
        cfg.apply_env_overrides();
        unsafe { std::env::remove_var("CMDOCK_TASK_READ_BATCH_MAX_UUIDS") };
        assert_eq!(
            cfg.task_read.batch_max_uuids, original,
            "zero must be ignored — would otherwise make every batch fail with TOO_MANY_UUIDS"
        );

        // --- task_read.batch_max_uuids env override (non-numeric — must be ignored) ---
        let mut cfg = default_config();
        let original = cfg.task_read.batch_max_uuids;
        unsafe { std::env::set_var("CMDOCK_TASK_READ_BATCH_MAX_UUIDS", "not-a-number") };
        cfg.apply_env_overrides();
        unsafe { std::env::remove_var("CMDOCK_TASK_READ_BATCH_MAX_UUIDS") };
        assert_eq!(cfg.task_read.batch_max_uuids, original);
    }

    #[test]
    fn test_task_read_section_default_when_omitted() {
        let _env_guard = env_test_guard();
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("no_task_read.toml");
        std::fs::write(
            &path,
            "[server]\nhost = \"127.0.0.1\"\nport = 3000\ndata_dir = \"./d\"\n",
        )
        .unwrap();
        for var in &["CMDOCK_TASK_READ_BATCH_MAX_UUIDS"] {
            unsafe { std::env::remove_var(var) };
        }
        let cfg = ServerConfig::load(&path).unwrap();
        assert_eq!(
            cfg.task_read.batch_max_uuids, 100,
            "default cap is 100 per task-read-contract.md § Configuration Model"
        );
    }

    #[test]
    fn test_task_read_section_explicit_toml_value() {
        let _env_guard = env_test_guard();
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("with_task_read.toml");
        std::fs::write(
            &path,
            r#"
[server]
...
"#,
        ...
        let cfg = ServerConfig::load(&path).unwrap();
        assert_eq!(cfg.task_read.batch_max_uuids, 50);
    }
}

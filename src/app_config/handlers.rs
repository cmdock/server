...
pub async fn get_app_config(
    ...
    // Phase 4 entry-point gate: ensure pre-feature task-keys backfill has
    // run for this user. Fast-path is a single DashMap lookup; slow-path
    // runs synchronously under the per-user mutation lock. Wired here so
    // clients hitting `/api/app-config` first (the iOS app's startup
    // flow) trigger backfill before the first task list call.
    if let Err(e) =
        crate::task_keys::backfill::ensure_user_task_keys_migrated(&state, &auth.user_id).await
    {
        tracing::error!(
            error = %e,
            user_id = %auth.user_id,
            "task-keys backfill failed in get_app_config",
        );
        return Err(StatusCode::INTERNAL_SERVER_ERROR);
    }

    // Contexts must reconcile before views so any future view-write validator
    // sees the seeded contexts that builtin views reference via context_id.
    if let Err(e) =
        crate::views::defaults::reconcile_default_contexts(state.store.as_ref(), &auth.user_id)
            .await
    {
        tracing::warn!(
            "Failed to reconcile default contexts for {} during app-config load: {e}",
            auth.user_id
        );
    }
...
pub async fn list_contexts(
    ...
    {
        tracing::warn!(
            "Failed to reconcile default contexts for {}: {e}",
            auth.user_id
        );
    }
...
pub async fn upsert_context(
    ...
    // Mirror upsert_view: detect builtin edits so reconciliation knows whether
    // the user has customised a seeded context (and won't clobber on version bump).
    let existing = state
        .store
        .list_contexts_all(&auth.user_id)
        .await
        .unwrap_or_default();
    let existing_ctx = existing.iter().find(|c| c.id == id);
    let is_builtin_edit = existing_ctx.is_some_and(|c| c.origin == "builtin");
    ...
    let record = ContextRecord {
        ...
        color: body.color,
        icon: body.icon,
        sort_order: existing_ctx.map(|c| c.sort_order).unwrap_or(0),
        origin: if is_builtin_edit {
            "builtin".to_string()
        } else {
            "user".to_string()
        },
        // Re-creating a hidden builtin (un-hiding via PUT) counts as a modification.
        user_modified: is_builtin_edit,
        hidden: false,
        template_version: existing_ctx.map(|c| c.template_version).unwrap_or(0),
    };

...
def tw_run(config: Config, directory: str, *args: str) -> subprocess.CompletedProcess[str]:
    ...
    env = {**os.environ, "TASKRC": f"{directory}/taskrc", "TASKDATA": f"{directory}/data"}
    return subprocess.run(
        ["task", *args],
        check=False,
        capture_output=True,
...
def run_restart_latency(config: Config) -> None:
    ...
    # Staging healthz includes Caddy + internal-network overhead; observed
    # baseline is ~140-150ms even when the server is healthy. Keep this bound
    # tight enough to catch gross regressions without failing on topology cost.
    record(
        health_p50 < 250,
        f"E2E-10: Healthz p50 < 250ms ({health_p50}ms)",
        f"E2E-10: Healthz p50 < 250ms (got {health_p50}ms)",
    )

...
# ═══════════════════════════════════════════════════
# TEST 13: Deleted merged sync DB → fresh start
# ═══════════════════════════════════════════════════
info "Test 13: Deleted merged sync DB → server recovers"

# Bob has 51 tasks. Delete his gateway-served merged sync DB.
BOB_SYNC_DB="$SERVER_DATA/users/$USER2_ID/merged/sync.sqlite"
if [[ -f "$BOB_SYNC_DB" ]]; then
    rm -f "$BOB_SYNC_DB" "$BOB_SYNC_DB-wal" "$BOB_SYNC_DB-shm"

    # Bob should be able to sync again (starts fresh chain)
    if $alias_task_bob sync 2>&1; then
        ok "Bob re-synced after merged sync DB deletion"
    else
        fail "Bob failed to re-sync after merged sync DB deletion"
    fi
    ...
    skip "Bob's merged sync DB not found"
fi

...
cleanup() {
    ...
    if [[ -n "$SERVER_PID" ]] && kill -0 "$SERVER_PID" 2>/dev/null; then
        ...
        curl -s --max-time 5 "http://127.0.0.1:$SERVER_PORT/metrics" 2>/dev/null | \
            grep -E "^(http_requests_total|http_request_duration|replica_open|replica_operation|config_db_quer|config_call_seconds|config_store_commit_seconds|filter_|sqlite_busy|http_requests_in_flight|llm_|replica_count|sync_)" | \
            sort || true
        echo
...
# Generate a random 32-byte master key (hex-encoded) for gateway-bound
# TaskChampion device credentials and sync payload translation during load
# testing.
...
ok "Master key generated (merged sync gateway ENABLED)"

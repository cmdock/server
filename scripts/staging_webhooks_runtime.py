...
def webhook_delivery_succeeds(config: Config) -> None:
    ...
    def has_signed_created() -> bool:
        return any(
            event_body(event).get("event") == "task.created"
    ...
    def has_priority_change() -> bool:
        return any(
            event_body(event).get("event") == "task.modified"
            and "priority" in (event_body(event).get("changed_fields") or [])
            for event in receiver_events(config)
        )
    ...
    def has_test_event() -> bool:
        return any(event_body(event).get("event") == "webhook.test" for event in receiver_events(config))
